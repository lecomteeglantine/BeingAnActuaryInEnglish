(function(){
  ...
  if(vocab.length && Object.values(els).every(Boolean)){
    function render(i){const v=vocab[((i%vocab.length)+vocab.length)%vocab.length];els.word.textContent=v.w;els.ipa.textContent=v.ipa;els.def.textContent=v.def;els.fr.textContent=v.fr;els.cat.textContent=v.cat;const ill=document.getElementById('wodIllustration');if(ill)ill.textContent=['📊','📈','🧮','🛡️','💷'][i%5];const listen=document.getElementById('wodListen');if(listen)listen.onclick=()=>speakWord(v.w+' — '+v.ex)}
