...
(function(){
  ...
  const KEY = 'actuarial_group_draft_r26';
  const OLD_KEYS = ['actuarial_group_draft_r25','actuarial_group_draft_r24','actuarial_group_draft_r23','actuarial_group_draft_r22','actuarial_group_draft_r21','actuarial_group_draft_v4_final_audit','actuarial_group_draft_v3_audit'];
  const mount = $('#gameMount');
  const progressFill = $('#progressFill');
  const dots = $$('#progressDots li');
  const soundBtn = $('#soundToggle');
  const defaultState = () => ({stage:1, soundOn:true, client:null, expertise:[], team:[], tools:[], identity:[], challengeChoice:null, firmName:'', promise:'', challengeId:null});
  let state;
  let soundOn = true;
  let voice = null;
  ...
  function normaliseState(raw){
    ...
    // Older builds did not store a challenge id. Because the challenge rules are unchanged,
    // attach the current deterministic id during migration. If a stored id disagrees, invalidate it.
    if(s.challengeChoice !== null){
      if(s.challengeId && s.challengeId !== currentChallenge.id) s.challengeChoice = null;
      s.challengeId = s.challengeChoice === null ? null : currentChallenge.id;
    } else s.challengeId = null;
    ...
    // Gate progression sequentially. A partially corrupted save must never
    // skip an earlier required round just because later arrays contain data.
    let maxStage = 1;
    if(s.client){
      maxStage = 2;
        ...
        if(s.team.length===2 && 100-spentBeforeTools>=18){
          ...
          if(s.tools.length===2 && total<=100){
            ...
            if(s.identity.length===2){
              maxStage = 7; // Review (6) and Client Alert (7) are now reachable.
  ...
  function toggleChoice(group, id, limit){
    ...
    lastChoiceAt = now;
    if(group==='client'){
      if(state.client !== id){ state.challengeChoice = null; state.challengeId = null; }
      state.client = id;
    ...
    state[group] = canonicalIds(group, arr);
    if(group==='expertise' || group==='client'){ state.challengeChoice = null; state.challengeId = null; }
    save(); render();
  }
  ...
  window.addEventListener('pageshow', event=>{
    if(event.persisted){ audioCtx = null; updateSound(); requestAnimationFrame(keepActiveProgressVisible); }
  });
