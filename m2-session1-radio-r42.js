...
const STORAGE='actuarial-radio-r42-state';
...
let activeAudio=null; let finalTimer=null; let finalRunId=0;
...
function runFinal(){
 stopAudio();stopFinalTimer();const runId=++finalRunId;playSfx('sfx-jingle.mp3');let time=60,q=0,score=0;
 const renderQ=()=>{
  if(runId!==finalRunId)return;if(time<=0){finalTimeout(score,q,runId);return;}if(q>=finalQs.length){finishFinal(score,runId);return;}
...
function resetGame(){finalRunId++;stopAudio();stopFinalTimer();state={mission:0,lives:3,correct:0,total:0,restore:0,badges:[],effects:state.effects,finished:false};try{localStorage.removeItem(STORAGE)}catch(_e){}els.start.textContent='Enter the control room →';els.brief.hidden=false;els.shell.hidden=true;updateHUD();window.scrollTo({top:0,behavior:'smooth'});}
