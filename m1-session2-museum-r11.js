(() => {
  'use strict';
  // M1 Session 2 museum engine · R11 zero-bug functional hardening

  const STORAGE_KEY = 'being-an-actuary-m1-session2-museum-r11';
  const LEGACY_KEYS = ['being-an-actuary-m1-session2-museum-r10','being-an-actuary-m1-session2-museum-r9','being-an-actuary-m1-session2-museum-r8','being-an-actuary-m1-session2-museum-r7','being-an-actuary-m1-session2-museum-r6','being-an-actuary-m1-session2-museum-r5','being-an-actuary-m1-session2-museum-r4','being-an-actuary-m1-session2-museum-r3','being-an-actuary-m1-session2-museum-r2','being-an-actuary-m1-session2-museum-r1'];
  const MAX_SCORE = 32;
  const galleryThumbs = [
    'assets/session2-museum/gallery1-birth.svg?v=r11',
    'assets/session2-museum/gallery2-probability.svg?v=r11',
    'assets/session2-museum/gallery3-radicals.svg?v=r11',
    'assets/session2-museum/gallery4-final.svg?v=r11'
  ];
  ...
  const galleries = [
    {
      ...
      objects: [
        ['assets/session2-museum/ethical-society.svg?v=r11', 'Early insurance society'],
        ['assets/session2-museum/mores-title.svg?v=r11', 'The title “actuary”'],
        ['assets/session2-museum/morgan-ledger.svg?v=r11', 'Professional actuarial work']
      ],
      featuredQuestion: 0,
    ...
    },
    {
      ...
      objects: [
        ['assets/session2-museum/bayes-price.svg?v=r11', 'Bayes & Price manuscript'],
        ['assets/session2-museum/mortality-table.svg?v=r11', 'Population table'],
        ['assets/session2-museum/premium-ledger.svg?v=r11', 'Premium ledger']
      ],
      featuredQuestion: 0,
    ...
    },
    {
      ...
      objects: [
        ['assets/session2-museum/price-pamphlet.svg?v=r11', 'Price’s political writing'],
        ['assets/session2-museum/letters-network.svg?v=r11', 'Atlantic correspondence'],
        ['assets/session2-museum/priestley-science.svg?v=r11', 'Science & dissent']
      ],
      featuredQuestion: 1,
    ...
    },
    {
      ...
      objects: [
        ['assets/session2-museum/concept-wheel.svg?v=r11', 'Ideas connected'],
        ['assets/session2-museum/gallery4-final.svg?v=r11', 'Four-frame wall'],
        ['assets/session2-museum/museum-hero.svg?v=r11', 'Opening night']
      ],
      featuredQuestion: 0,
  ...
  function loadState() {
    ...
    const hadFinalIntent = merged.finalReached === true;
    merged.started = Boolean(merged.started === true && [3,4].includes(merged.playerCount));
    merged.setupConfirmed = Boolean(merged.setupConfirmed === true && merged.started);
    ...
    merged.sound = typeof parsed.sound === 'boolean' ? parsed.sound : true;
    merged.finalScript = typeof merged.finalScript === 'string' ? merged.finalScript.slice(0, 2200) : '';
    merged.pitchNotes = merged.pitchNotes.map((note) => note.slice(0, 600));
    merged.pitchEdited = Array.isArray(parsed.pitchEdited) && parsed.pitchEdited.length === galleries.length
      ? parsed.pitchEdited.map((x) => x === true)
      : merged.pitchNotes.map((note) => Boolean(note.trim()));
    ...
    merged.reviewFromFinal = Boolean(((parsed.reviewFromFinal === true) || (hadFinalIntent && !merged.finalReached)) && merged.started && !merged.finalReached);
    return merged;
  }
  ...
  const sfxAssets = {
    on: 'assets/session2-museum/audio/sound-on.wav?v=r11',
    select: 'assets/session2-museum/audio/select.wav?v=r11',
    save: 'assets/session2-museum/audio/save.wav?v=r11',
    complete: 'assets/session2-museum/audio/gallery-complete.wav?v=r11',
    final: 'assets/session2-museum/audio/final.wav?v=r11',
    timer: 'assets/session2-museum/audio/timer.wav?v=r11',
    attention: 'assets/session2-museum/audio/attention.wav?v=r11'
  };
  ...
  function testSound() {
    ...
    preloadSfx();
    playSfx('on').then((played) => { if (state.sound) showStatus(played ? 'Audio test: museum chime played.' : 'This browser is blocking audio playback. Check the tab/site sound permission.'); });
  }

  function speak(text) {
    if (!state.sound) { showStatus('Sound is off.'); return; }
    if (!('speechSynthesis' in window) || typeof window.SpeechSynthesisUtterance !== 'function') {
      showStatus('Curator voice is not available in this browser. The game sound effects still work.');
      return;
    }
    try {
  ...
  function withActionLock(key, fn, ms=420) {
    ...
    const token = ++actionLockToken;
    actionLocks.set(key, token);
    try { fn(); } finally {
      setTimeout(() => {
        // A timer from an older DOM must never unlock a newer action that reused
        // the same key after a rerender.
        if (actionLocks.get(key) === token) actionLocks.delete(key);
      }, ms);
    }
  }
  ...
  function speakerForGallery(gi) {
    ...
    if (state.playerCount === 4) {
      return ['Speaker 1 · Head Curator', 'Speaker 2 · Historical Evidence Officer', 'Speaker 3 · Actuarial Relevance Officer', 'Speaker 4 · Visitor Experience Designer'][gi];
    }
    return ['Speaker 1 · Head Curator', 'Speaker 2 · Historical Evidence Officer', 'Speaker 2 · Historical Evidence Officer', 'Speaker 3 · Actuarial Story Officer'][gi];
  }
  ...
  function renderGallery() {
    ...
    galleryView.innerHTML = `
        ...
        ${g.questions.map((q, qi) => `
          <section class="question-card" aria-labelledby="q-${gi}-${qi}">
            ...
            <div class="options-grid" role="radiogroup" aria-label="${escapeHtml(q.prompt)}">
              ${q.options.map((opt, oi) => { const selected = state.answers[gi][qi] === oi; const tab = state.answers[gi][qi] === null ? oi === 0 : selected; return `<button class="option-card${selected ? ' selected' : ''}" type="button" role="radio" aria-checked="${selected}" tabindex="${tab ? '0' : '-1'}" data-q="${qi}" data-o="${oi}"><span class="option-letter">${String.fromCharCode(65+oi)}</span>${escapeHtml(opt.text)}</button>`; }).join('')}
            </div>
          </section>`).join('')}
      </div>
      <section class="pitch-builder-card" aria-labelledby="pitchBuilderHeading">
        ...
        <textarea class="pitch-textarea" id="galleryPitchNote" maxlength="600" ${allAnswered(gi) ? '' : 'disabled'} placeholder="Choose all four answers first. A suggested speaking note will then appear here.">${escapeHtml(state.pitchNotes[gi])}</textarea>
  ...
  function speakerPlan() {
    ...
    if (p === 4) return [
      ...
      ['Speaker 2 · Historical Evidence Officer', 'Present Gallery II and give one clear historical sequence. Aim for about 20–25 seconds.'],
      ['Speaker 3 · Actuarial Relevance Officer', 'Present Gallery III and explain why the wider context matters. Aim for about 20–25 seconds.'],
      ['Speaker 4 · Visitor Experience Designer', 'Present Gallery IV, then deliver the conclusion. Aim for about 25–30 seconds.']
    ];
    return [
      ['Speaker 1 · Head Curator', 'Open the exhibition, then present Gallery I. Aim for about 30–35 seconds.'],
      ['Speaker 2 · Historical Evidence Officer', 'Present Galleries II and III, using one clear historical sequence. Aim for about 35–40 seconds.'],
  ...
  function renderFinal() {
    ...
    finalView.innerHTML = `
      <div class="final-hero">
        <img src="assets/session2-museum/gallery4-final.svg?v=r11" alt="Four framed exhibits on the final museum wall.">
    ...
    const updateScriptStats = () => {
      ...
      const tooShort = words > 0 && words < 170;
