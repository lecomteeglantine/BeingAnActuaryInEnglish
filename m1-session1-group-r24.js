...
(function(){
  ...
  const KEY = 'actuarial_group_draft_r24';
  const OLD_KEYS = ['actuarial_group_draft_r23','actuarial_group_draft_r22','actuarial_group_draft_r21','actuarial_group_draft_v4_final_audit','actuarial_group_draft_v3_audit'];
  ...
  function statusInfo(){
    ...
    if(state.stage === 7 && state.challengeChoice !== 0) return ['Alert pending','status-warning'];
    if(state.stage === 7 && state.challengeChoice === 0) return ['Alert cleared','status-good'];
  ...
  window.addEventListener('pagehide', ()=>{ try{ window.speechSynthesis && window.speechSynthesis.cancel(); }catch(_){} try{ audioCtx && audioCtx.close && audioCtx.close(); }catch(_){} });
