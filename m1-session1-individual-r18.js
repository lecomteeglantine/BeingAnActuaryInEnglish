...
function resetTransientSelections(){selectedWord='';selectedReplacement='';}
...
function renderBrief(){
 ...
 <div class="word-bank">${briefWords.map(w=>`<button class="word-chip ${selectedWord===w?'active':''} ${used.includes(w)?'used':''}" data-word="${w}" ${state.earned[1]?'disabled':''}>${w}</button>`).join('')}</div>
 ...
 <div class="stage-footer"><div class="stage-score">${correctCount}/8 lines correct</div><div class="stage-actions"><button class="secondary-game" id="checkBrief" ${state.earned[1]?'disabled':''}>Check brief</button>${completionButton(correctCount===8,'Send to manager')}</div></div>`);
...
function renderPron(){
 ...
 <div class="gate-grid">${pron.map((w,i)=>`<div class="gate-card"><div class="gate-head"><div class="gate-word"><span class="micro-label">CHECKPOINT ${i+1}</span><strong>${w.word}</strong><small>${w.ipa}</small></div><button class="listen-btn" data-say="${w.word}">🔊 Listen</button></div><div class="choice-grid">${w.opts.map((o,j)=>`<button class="choice ${state.pron[w.id]&&j===w.correct?'correct':''}" data-pron="${w.id}" data-choice="${j}" ${state.pron[w.id]?'disabled':''}>${o}</button>`).join('')}</div><div class="feedback" id="fb-${w.id}">${state.pron[w.id]?w.note:''}</div></div>`).join('')}</div>${footer(`${done}/8 access checks cleared`,done===8,'Open client floor')}`);
...
function renderBoard(){
 ...
 <div class="replacement-deck">${replacements.map((r,i)=>`<button class="replacement ${selectedReplacement===r?'active':''} ${used.includes(r)?'used':''}" data-replace="${i}" ${state.earned[7]?'disabled':''}>${r}</button>`).join('')}</div>
 ...
 <div class="stage-footer"><div class="stage-score">${correct}/4 board lines correct</div><div class="stage-actions"><button class="secondary-game" id="checkSlide" ${state.earned[7]?'disabled':''}>Run clarity check</button>${completionButton(correct===4,'Send to board')}</div></div>
