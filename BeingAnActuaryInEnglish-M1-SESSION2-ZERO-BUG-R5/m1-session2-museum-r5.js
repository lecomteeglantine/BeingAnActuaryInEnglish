(() => {
  'use strict';
  // M1 Session 2 museum engine · R5 functional stability audit

  const STORAGE_KEY = 'being-an-actuary-m1-session2-museum-r5';
  const LEGACY_KEYS = ['being-an-actuary-m1-session2-museum-r4','being-an-actuary-m1-session2-museum-r3','being-an-actuary-m1-session2-museum-r2','being-an-actuary-m1-session2-museum-r1'];
  const MAX_SCORE = 32;
  const galleryThumbs = [
  ...
  function resetTimer() { resetTimerSilently(); playTone('select'); }
  ...
  window.addEventListener('pagehide', () => { clearActionLocks(); stopTimer(); if ('speechSynthesis' in window) window.speechSynthesis.cancel(); });
