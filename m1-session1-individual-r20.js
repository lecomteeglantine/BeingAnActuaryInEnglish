...
const KEY='actuary_m1_s1_northstar_v20';
const LEGACY_KEYS=['actuary_m1_s1_northstar_v17_usability'];
...
let meetingLockUntil=0, labLockUntil=0, clearing=false;
...
resetBtn.addEventListener('click',()=>{if(!confirm('Restart Actuary for a Day from the beginning?'))return;if('speechSynthesis'in window)speechSynthesis.cancel();sessionStorage.removeItem(KEY);location.reload();});
soundBtn.addEventListener('click',()=>{state.soundOn=!state.soundOn;if(!state.soundOn&&'speechSynthesis'in window)speechSynthesis.cancel();soundBtn.textContent=state.soundOn?'🔊 Sound on':'🔈 Sound off';soundBtn.setAttribute('aria-pressed',String(state.soundOn));save();primeVoice();});
...
function renderPron(){
 ...
 $$('[data-say]',mount).forEach(b=>b.onclick=()=>{if(!speak(b.dataset.say)){const card=b.closest('.gate-card'),fb=card&&$('.feedback',card);if(fb&&!speechOK)fb.textContent='Audio is not available in this browser. You can still use the IPA and stress patterns.';}});
...
function render(){updateHUD();soundBtn.textContent=state.soundOn?'🔊 Sound on':'🔈 Sound off';soundBtn.setAttribute('aria-pressed',String(state.soundOn));({1:renderBrief,2:renderPron,3:renderCases,4:renderLab,5:renderInbox,6:renderMeeting,7:renderBoard}[state.stage]||renderBrief)();requestAnimationFrame(keepActiveMapVisible);}
setModal(false);
render();
})();
