(() => {
  ...
  const STORAGE_KEY = 'being-an-actuary-m1-session2-museum-r1';
  const LEGACY_KEYS = [];
  const MAX_SCORE = 32;
  const galleryThumbs = [
  ...
  function loadState() {
    ...
    merged.finalReached = Boolean(merged.finalReached && merged.completed.every(Boolean));
    ...
    merged.reviewFromFinal = Boolean(merged.reviewFromFinal);
    return merged;
  }

  function saveState() {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); } catch { /* local saving may be unavailable */ }
  }
  ...
  function suggestedPitch(gi) {
    ...
    const notes = [
      ...
      `Our second exhibit is ${exhibit}. Price published Bayes’s work, which had remained unknown until then. Probability, demography and premiums helped make uncertainty increasingly measurable and usable in insurance decisions.`,
      ...
      `Our final exhibit is ${exhibit}. Together, the galleries show that actuarial science emerged through insurance, probability, professional roles and a wider world of political, philosophical and religious ideas.`
    ];
    return notes[gi];
  }
    ...
    wrap.querySelectorAll('[data-gallery-jump]').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        if (state.completed[i] || i <= firstIncompleteIndex()) {
          ...
          state.reviewFromFinal = wasFinal;
  ...
  function renderNotebook() {
    ...
    const words = state.pitchNotes.reduce((sum, n) => sum + countWords(n), 0) + (state.pitchNotes.some((n) => n.trim()) ? 36 : 0);
    ...
    if (words === 0) hint.textContent = 'Aim for roughly 150–210 words in total.';
    else if (seconds > 112) hint.textContent = 'Your draft is getting long. Trim repeated facts so you have breathing room under two minutes.';
    ...
    else hint.textContent = 'Good pace: keep the final team pitch under two minutes.';
  }
    ...
    setupView.innerHTML = `
      <div class="setup-card">
        ...
        <div class="player-choice-grid">
          <button class="player-choice" type="button" data-players="3" aria-pressed="${state.playerCount === 3}"><strong>3 students</strong><span>Three roles · approx. 35–40 seconds each in the final pitch.</span></button>
          <button class="player-choice" type="button" data-players="4" aria-pressed="${state.playerCount === 4}"><strong>4 students</strong><span>Four roles · approx. 25–30 seconds each in the final pitch.</span></button>
        </div>
        <div id="rolePreview"></div>
        <div class="setup-warning"><strong>Pitch rule:</strong> every student must speak. Your team has <strong>2 minutes maximum in total</strong>. The Pitch Notebook will build the structure gallery by gallery.</div>
  ...
  function renderGallery() {
    ...
    galleryView.innerHTML = `
      ...
      <section class="pitch-builder-card" aria-labelledby="pitchBuilderHeading">
        ...
        <div class="pitch-tools"><small id="galleryPitchCount">${countWords(state.pitchNotes[gi])} words</small><button class="museum-btn subtle" id="suggestPitchBtn" type="button" ${allAnswered(gi) ? '' : 'disabled'}>Use suggested wording</button></div>
      </section>
      ...
      <div class="gallery-actions">
        ...
        <button class="museum-btn primary" id="saveGalleryBtn" type="button" ${allAnswered(gi) ? '' : 'disabled'}>${state.completed[gi] ? 'Save changes' : 'Save gallery'} ${gi === 3 ? '→ Opening night' : '→ Next gallery'}</button>
      </div>`;
    ...
    galleryView.querySelectorAll('.option-card').forEach((btn) => btn.addEventListener('click', () => {
      ...
      const old = state.answers[gi][qi];
      state.answers[gi][qi] = oi;
      if (old !== oi) state.completed[gi] = false;
    ...
    note.addEventListener('input', () => {
      ...
      saveState();
      $('galleryPitchCount').textContent = `${countWords(note.value)} words`;
      renderNotebook();
    });
  ...
  function saveGallery(gi) {
    ...
    if (!state.pitchNotes[gi].trim()) { state.pitchNotes[gi] = suggestedPitch(gi); state.pitchEdited[gi] = false; }
    state.completed[gi] = true;
    state.finalScript = '';
    playTone(gi === galleries.length - 1 ? 'complete' : 'save');
    if (state.reviewFromFinal && state.completed.every(Boolean)) {
      state.finalReached = true;
      state.reviewFromFinal = false;
      state.currentGallery = galleries.length - 1;
    } else if (gi === galleries.length - 1) {
  ...
  function speakerPlan() {
    ...
    if (p === 4) return [
      ['Speaker 1 · Head Curator', 'Open the exhibition and present Gallery I. Aim for about 25–30 seconds.'],
      ['Speaker 2 · Evidence Officer', 'Present Gallery II and give the strongest historical sequence. Aim for about 25–30 seconds.'],
      ['Speaker 3 · Relevance Officer', 'Present Gallery III and explain why the wider intellectual context matters. Aim for about 25–30 seconds.'],
      ['Speaker 4 · Visitor Designer', 'Present Gallery IV and deliver the conclusion. Aim for about 25–30 seconds.']
    ];
    return [
      ['Speaker 1 · Head Curator', 'Open the exhibition and present Gallery I. Aim for about 35–40 seconds.'],
      ['Speaker 2 · Evidence Officer', 'Present Galleries II and III, using one clear historical sequence. Aim for about 35–40 seconds.'],
      ['Speaker 3 · Actuarial Story Officer', 'Present Gallery IV and deliver the conclusion. Aim for about 35–40 seconds.']
    ];
  }
  ...
  function renderFinal() {
    ...
    if (!state.finalScript.trim()) state.finalScript = defaultFinalScript();
    ...
    finalView.innerHTML = `
      ...
      <section class="final-pitch-section">
        ...
        <p><strong>Every student speaks.</strong> Do not list your objects. Tell one connected story: where actuarial science began, how ideas about uncertainty developed, why the wider historical context matters, and what visitors should remember.</p>
        ...
        <textarea class="final-script" id="finalScript" aria-label="Editable final team pitch">${escapeHtml(state.finalScript)}</textarea>
    ...
    const updateScriptStats = () => {
      ...
      $('finalTimeEstimate').className = sec > 115 ? 'timer-warning' : '';
    };
    updateScriptStats();
  ...
  function toggleTimer() {
    if (timerInterval) { clearInterval(timerInterval); timerInterval = null; renderTimer(); return; }
