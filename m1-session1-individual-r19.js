...
const KEY='actuary_m1_s1_northstar_v17_usability'; // R18 keeps R17 progress compatible
...
let state=load(); let selectedWord=''; let selectedReplacement=''; let voice=null; let audioCtx=null; let lastFocus=null;
...
function sanitizeState(raw){
 ...
 s.meetingWrong=typeof s.meetingWrong==='string'?s.meetingWrong:'';
...
function completionButton(nextEnabled,label='Mission complete'){const cleared=!!state.earned[state.stage],resume=maxUnlocked();if(cleared&&state.stage<resume)return `<button class="primary-game" id="completeMission">Return to active mission →</button>`;return `<button class="primary-game" id="completeMission" ${nextEnabled&&!cleared?'':'disabled'}>${cleared?'Mission already cleared':label+' →'}</button>`;}
...
function awardAndClear(num,title,text){if(state.earned[num]){const target=maxUnlocked();if(target!==state.stage){resetTransientSelections();state.stage=target;save();render();focusStageHeading();scrollToGame();}return;}state.earned[num]=true;state.rep=Object.keys(state.earned).reduce((sum,k)=>sum+(REWARDS[Number(k)]||0),0);save();sfx('clear');$('#clearTitle').textContent=title;$('#clearText').textContent=text;$('#clearReward').textContent='+'+REWARDS[num];lastFocus=document.activeElement;overlay.hidden=false;document.body.classList.add('modal-open');continueBtn.dataset.from=String(num);updateHUD();requestAnimationFrame(()=>continueBtn.focus());}
continueBtn.addEventListener('click',()=>{const from=Number(continueBtn.dataset.from)||state.stage;overlay.hidden=true;document.body.classList.remove('modal-open');resetTransientSelections();if(from<7){state.stage=Math.max(state.stage,from+1);render();focusStageHeading();scrollToGame();}else{render();focusStageHeading();}lastFocus=null;});
...
window.addEventListener('pagehide',()=>{if('speechSynthesis'in window)speechSynthesis.cancel();document.body.classList.remove('modal-open');});
...
function renderBrief(){
 ...
 <div class="game-grid">${briefSlots.map((x,i)=>{const assigned=state.brief[x.id]||'drop term';let cls='drop-line';if(state.briefChecked)cls+=assigned===x.answer?' correct':' wrong';return `<button class="${cls}" data-slot="${x.id}" ${state.earned[1]?'disabled':''}><span class="micro-label">LINE ${i+1}</span><div>${x.text.replace('___',`<span>${escapeHtml(assigned)}</span>`)}</div></button>`;}).join('')}</div>
 ...
 <div class="stage-footer"><div class="stage-score">${correctCount}/8 lines correct</div><div class="stage-actions"><button class="secondary-game" id="checkBrief" ${state.earned[1]?'disabled':''}>Check brief</button>${completionButton(state.briefChecked&&correctCount===8,'Send to manager')}</div></div>`);
...
function renderPron(){
 ...
 <div class="gate-grid">${pron.map((w,i)=>`<div class="gate-card"><div class="gate-head"><div class="gate-word"><span class="micro-label">CHECKPOINT ${i+1}</span><strong>${w.word}</strong><small>${w.ipa}</small></div><button type="button" class="listen-btn" data-say="${w.word}">🔊 Listen</button></div><div class="choice-grid">${w.opts.map((o,j)=>`<button type="button" class="choice ${state.pron[w.id]&&j===w.correct?'correct':''}" data-pron="${w.id}" data-choice="${j}" ${state.pron[w.id]?'disabled':''}>${o}</button>`).join('')}</div><div class="feedback" id="fb-${w.id}">${state.pron[w.id]?w.note:''}</div></div>`).join('')}</div>${footer(`${done}/8 access checks cleared`,done===8,'Open client floor')}`);
...
function renderLab(){
 ...
 <div class="stage-footer"><div class="stage-score">${complete?'Risk Lab cleared':state.labTrayOK?'Tray accepted — solve the shock':'Select 5 model inputs'}</div><div class="stage-actions">${!state.labTrayOK?'<button class="secondary-game" id="checkLab">Run model check</button>':''}${completionButton(complete,'Send model note')}</div></div>`);
...
function renderMeeting(){
 ...
 $$('[data-meet]',mount).forEach(b=>b.onclick=()=>{const i=Number(b.dataset.meet);if(turn.answers[i][1]){state.meeting.push(i);state.meetingWrong='';sfx('ok');save();renderMeeting();}else{state.meetingWrong='The room goes quiet. That answer is either too confident, too vague or simply wrong. Try another response.';sfx('bad');save();renderMeeting();}});
...
function renderBoard(){
 ...
 <div class="stage-footer"><div class="stage-score">${correct}/4 board lines correct</div><div class="stage-actions"><button class="secondary-game" id="checkSlide" ${state.earned[7]?'disabled':''}>Run clarity check</button>${completionButton(state.slideChecked&&correct===4,'Send to board')}</div></div>
