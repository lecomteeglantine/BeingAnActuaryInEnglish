...
const locationEl=$('#officeLocation'), missionEl=$('#missionNumber'), statusEl=$('#statusText');
...
const mapTipText=$('.map-tip p'), pageRegions=$$('header,main,footer');
const KEY='actuary_m1_s1_northstar_v22';
const LEGACY_KEYS=['actuary_m1_s1_northstar_v20','actuary_m1_s1_northstar_v19','actuary_m1_s1_northstar_v18','actuary_m1_s1_northstar_v17_usability','actuary_m1_s1_northstar_v16_audit'];
const REWARDS={1:12,2:14,3:16,4:14,5:14,6:15,7:15};
const STAGES={
 1:{place:'Brief Desk',status:'Client file corrupted'},
 2:{place:'Access Gate',status:'Audio clearance required'},
 3:{place:'Case Room',status:'Three client files waiting'},
 4:{place:'Risk Lab',status:'Model tray incomplete'},
 5:{place:'Team Inbox',status:'Client pack at risk'},
 6:{place:'Client Meeting',status:'Meeting live'},
 7:{place:'Boardroom',status:'Presentation pending'}
};
...
let storageOK=true; let state=load(); let selectedWord=''; let selectedReplacement=''; let voice=null; let audioCtx=null; let lastFocus=null;
let meetingLockUntil=0, lastLabIndex=-1, lastLabAt=0, clearing=false;
const MEETING_WRONG='The room goes quiet. That answer is either too confident, too vague or simply wrong. Try another response.';
// Progress persists for the lifetime of this browser tab. Only the explicit Restart button clears it.
...
const inbox=[
{id:'i1',from:'Pricing manager',tag:'Pricing',text:'An actuary helps a company make informed decisions under uncertainty.',good:true,why:'Keep it: clear, accurate and client-friendly.'},
{id:'i2',from:'Intern',tag:'Team chat',text:'Actuaries predict the exact future.',good:false,why:'Archive it: actuaries estimate possible outcomes; they do not know the exact future.'},
{id:'i3',from:'Data team',tag:'Data quality',text:'Weak data can distort the model and the decision that follows.',good:true,why:'Keep it: data quality matters.'},
{id:'i4',from:'Office chat',tag:'Myth check',text:'Actuaries mainly make spreadsheets and reports look complicated.',good:false,why:'Archive it: the goal is useful insight, not complexity for its own sake.'},
{id:'i5',from:'Manager',tag:'Profession',text:'Actuaries can work in insurance, pensions, finance, consulting and risk teams.',good:true,why:'Keep it: that is a useful overview of the profession.'}];
const meeting=[
...
function sanitizeState(raw){
 const s={...defaultState(),...asObject(raw)};
 s.stage=Math.max(1,Math.min(7,Number(s.stage)||1));
 s.soundOn=s.soundOn!==false;
 s.earned=asObject(s.earned);
 s.brief=asObject(s.brief);
 const cleanBrief={};
 briefSlots.forEach(slot=>{const v=s.brief[slot.id];if(briefWords.includes(v))cleanBrief[slot.id]=v;});
 // A word can only occupy one slot. Keep the first occurrence if an old/corrupt state duplicated it.
 const seenWords=new Set();
 s.brief=Object.fromEntries(Object.entries(cleanBrief).filter(([,v])=>{if(seenWords.has(v))return false;seenWords.add(v);return true;}));
 s.briefChecked=!!s.briefChecked;
 const rawPron=asObject(s.pron); s.pron={}; pron.forEach(w=>{if(rawPron[w.id]===true)s.pron[w.id]=true;});
 const rawCases=asObject(s.caseDone); s.caseDone={};
 Object.keys(cases).forEach(id=>{const c=asObject(rawCases[id]);const clean={};if(c.q1===true)clean.q1=true;if(c.q2===true&&clean.q1)clean.q2=true;if(Object.keys(clean).length)s.caseDone[id]=clean;});
 s.casePanel=cases[s.casePanel]?s.casePanel:'motor';
 const rawLab=Array.isArray(s.lab)?s.lab:[];
 s.lab=[...new Set(rawLab.map(Number).filter(i=>Number.isInteger(i)&&i>=0&&i<labItems.length))].slice(0,5);
 const labCorrect=s.lab.length===5&&s.lab.every(i=>labItems[i][2]);
 s.labTrayOK=!!s.labTrayOK&&labCorrect;
 s.labShock=!!s.labShock&&s.labTrayOK;
 const rawInbox=asObject(s.inbox); s.inbox={}; inbox.forEach(m=>{if(rawInbox[m.id]===m.good)s.inbox[m.id]=m.good;});
 const rawMeeting=Array.isArray(s.meeting)?s.meeting:[]; s.meeting=[];
 for(let i=0;i<meeting.length&&i<rawMeeting.length;i++){const choice=Number(rawMeeting[i]);if(!Number.isInteger(choice)||!meeting[i].answers[choice]||meeting[i].answers[choice][1]!==true)break;s.meeting.push(choice);}
 s.meetingWrong=s.meetingWrong===MEETING_WRONG?MEETING_WRONG:'';
 const rawSlide=asObject(s.slide); s.slide={}; const seenReplacements=new Set();
 slideSlots.forEach(slot=>{const v=rawSlide[slot.id];if(replacements.includes(v)&&!seenReplacements.has(v)){s.slide[slot.id]=v;seenReplacements.add(v);}});
 s.slideChecked=!!s.slideChecked;
 const completed={
  1:briefSlots.every(x=>s.brief[x.id]===x.answer),
  2:pron.every(w=>s.pron[w.id]===true),
  3:Object.keys(cases).every(id=>s.caseDone[id]?.q1===true&&s.caseDone[id]?.q2===true),
  4:s.labTrayOK&&s.labShock,
  5:inbox.every(m=>s.inbox[m.id]===m.good),
  6:s.meeting.length===meeting.length,
  7:slideSlots.every(slot=>s.slide[slot.id]===slot.answer)
 };
 const earned={};
 for(let i=1;i<=7;i++){if(s.earned[i]===true&&completed[i]&&(i===1||earned[i-1]===true))earned[i]=true;else break;}
 s.earned=earned;
 s.rep=Object.keys(earned).reduce((sum,k)=>sum+(REWARDS[Number(k)]||0),0);
 let unlocked=1;for(let i=1;i<=7;i++){if(earned[i])unlocked=Math.min(7,i+1);else break;}
 s.stage=Math.min(s.stage,unlocked);
 return s;
}
...
function load(){
 try{
  ...
  if(!raw){for(const legacy of LEGACY_KEYS){raw=sessionStorage.getItem(legacy);if(raw)break;}}
  return {...defaultState(),...(JSON.parse(raw||'null')||{})};
 }catch(_){storageOK=false;return defaultState();}
}
...
function save(){
 ...
 try{
  ...
  LEGACY_KEYS.forEach(k=>{try{sessionStorage.removeItem(k);}catch(_){}});
...
function updateHUD(){const s=STAGES[state.stage]; locationEl.textContent=s.place; missionEl.textContent=state.stage; statusEl.textContent=state.earned[state.stage]?'Mission cleared':s.status; repEl.textContent=state.rep||0; meterEl.style.width=Math.min(100,state.rep||0)+'%'; levelEl.textContent=badgeLevel()[0]; mapItems.forEach(li=>{const n=Number(li.dataset.step); li.classList.toggle('active',n===state.stage); li.classList.toggle('done',!!state.earned[n]); const btn=$('button',li); btn.disabled=n>maxUnlocked(); btn.style.cursor=n<=maxUnlocked()?'pointer':'default';}); save(); if(mapTipText)mapTipText.textContent=storageOK?'Complete a mission to unlock the next room. Your progress stays in this browser tab until you press Restart.':'Progress cannot be saved in this browser mode. Keep this page open until you finish the game.';}
function stageFrame(num,title,subtitle,img,chips,body){return `<article class="stage-card"><div class="stage-visual"><img src="${img}" alt="Illustration for ${escapeHtml(title)}"><div class="visual-copy"><span class="mission-label">MISSION ${num} OF 7</span><h2 class="stage-heading" tabindex="-1">${title}</h2><p>${subtitle}</p><div class="visual-chip-row">${chips.map(c=>`<span>${c}</span>`).join('')}</div></div></div><div class="stage-body">${body}</div></article>`;}
...
function completionButton(nextEnabled,label='Mission complete'){const cleared=!!state.earned[state.stage],resume=maxUnlocked();if(cleared&&state.stage<resume)return `<button type="button" class="primary-game" id="completeMission">Return to active mission →</button>`;return `<button type="button" class="primary-game" id="completeMission" ${nextEnabled&&!cleared?'':'disabled'}>${cleared?'Mission already cleared':label+' →'}</button>`;}
function footer(score,nextEnabled,label='Mission complete'){return `<div class="stage-footer"><div class="stage-score">${score}</div><div class="stage-actions">${completionButton(nextEnabled,label)}</div></div>`;}
...
function resetTransientSelections(){selectedWord='';selectedReplacement='';state.meetingWrong='';}
function keepActiveMapVisible(){
 const list=$('#missionMap'),active=$('#missionMap li.active');
 if(!list||!active||!window.matchMedia('(max-width:1050px)').matches)return;
 const target=active.offsetLeft-(list.clientWidth-active.offsetWidth)/2;
 list.scrollTo({left:Math.max(0,target),behavior:'auto'});
}
function focusStageHeading(){const h=$('.stage-heading',mount);if(h)h.focus({preventScroll:true});}
function scrollToGame(){const layout=document.querySelector('.game-layout');if(!layout)return;const reduce=window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches;window.scrollTo({top:Math.max(0,layout.offsetTop-80),behavior:reduce?'auto':'smooth'});}
window.addEventListener('resize',()=>requestAnimationFrame(keepActiveMapVisible),{passive:true});
function setModal(open){
 overlay.hidden=!open;overlay.setAttribute('aria-hidden',String(!open));document.body.classList.toggle('modal-open',open);
 pageRegions.forEach(el=>{if(open)el.setAttribute('inert','');else el.removeAttribute('inert');});
}
function awardAndClear(num,title,text){
 if(clearing||!overlay.hidden)return;
 if(state.earned[num]){const target=maxUnlocked();if(target!==state.stage){resetTransientSelections();state.stage=target;save();render();focusStageHeading();scrollToGame();}return;}
 clearing=true;state.earned[num]=true;state.rep=Object.keys(state.earned).reduce((sum,k)=>sum+(REWARDS[Number(k)]||0),0);save();sfx('clear');$('#clearTitle').textContent=title;$('#clearText').textContent=text;$('#clearReward').textContent='+'+REWARDS[num];lastFocus=document.activeElement;setModal(true);continueBtn.dataset.from=String(num);updateHUD();requestAnimationFrame(()=>continueBtn.focus());
}
continueBtn.addEventListener('click',()=>{if(overlay.hidden)return;const from=Number(continueBtn.dataset.from)||state.stage;setModal(false);clearing=false;resetTransientSelections();if(from<7){state.stage=Math.max(state.stage,from+1);render();focusStageHeading();scrollToGame();}else{render();focusStageHeading();}lastFocus=null;});
overlay.addEventListener('keydown',e=>{if(e.key==='Tab'){e.preventDefault();continueBtn.focus();}});
mapItems.forEach(li=>$('button',li).addEventListener('click',()=>{const n=Number(li.dataset.step);if(n<=maxUnlocked()){resetTransientSelections();state.stage=n;render();focusStageHeading();scrollToGame();}}));
resetBtn.addEventListener('click',()=>{if(!confirm('Restart Actuary for a Day from the beginning?'))return;if('speechSynthesis'in window)speechSynthesis.cancel();sessionStorage.removeItem(KEY);LEGACY_KEYS.forEach(k=>{try{sessionStorage.removeItem(k);}catch(_){}});location.reload();});
soundBtn.addEventListener('click',()=>{state.soundOn=!state.soundOn;if(!state.soundOn&&'speechSynthesis'in window)speechSynthesis.cancel();updateSoundButton();save();primeVoice();});
window.addEventListener('pagehide',()=>{if('speechSynthesis'in window)speechSynthesis.cancel();setModal(false);clearing=false;});
...
function speak(text){if(!state.soundOn)return false;if(!('speechSynthesis'in window)||!('SpeechSynthesisUtterance'in window))return false;primeVoice();const u=new SpeechSynthesisUtterance(text);if(voice)u.voice=voice;u.lang=voice?.lang||'en-GB';u.rate=.94;if(speechSynthesis.speaking)speechSynthesis.cancel();speechSynthesis.speak(u);return true;}
...
function renderBrief(){
 ...
 mount.innerHTML=stageFrame(1,'Restore the client brief','Your first file is corrupted. Rebuild it from the word bank and send a clean version to your manager.','assets/session1/realistic/brief-desk.png',['Vocabulary puzzle','Tap-to-place','8 key terms'],`${dispatch('🗂️','Message from your manager','Select a term from the word bank, then click the line where it belongs. You can move terms until the brief is correct.')}
 <div class="word-bank">${briefWords.map(w=>`<button type="button" class="word-chip ${selectedWord===w?'active':''} ${used.includes(w)?'used':''}" data-word="${w}" aria-pressed="${selectedWord===w?'true':'false'}" ${state.earned[1]?'disabled':''}>${w}</button>`).join('')}</div>
 <div class="game-grid">${briefSlots.map((x,i)=>{const assigned=state.brief[x.id]||'drop term';let cls='drop-line';if(state.briefChecked)cls+=assigned===x.answer?' correct':' wrong';return `<button type="button" class="${cls}" data-slot="${x.id}" ${state.earned[1]?'disabled':''}><span class="micro-label">LINE ${i+1}</span><div>${x.text.replace('___',`<span>${escapeHtml(assigned)}</span>`)}</div></button>`;}).join('')}</div>
 ...
 <div class="stage-footer"><div class="stage-score">${correctCount}/8 lines correct</div><div class="stage-actions"><button type="button" class="secondary-game" id="checkBrief" ${state.earned[1]?'disabled':''}>Check brief</button>${completionButton(state.briefChecked&&correctCount===8,'Send to manager')}</div></div>`);
 $$('[data-word]',mount).forEach(b=>b.onclick=()=>{selectedWord=b.dataset.word;renderBrief();});
 $$('[data-slot]',mount).forEach(b=>b.onclick=()=>{if(!selectedWord){const fb=$('#briefFeedback');if(fb)fb.textContent='Select a term from the word bank first.';return;}Object.keys(state.brief).forEach(k=>{if(state.brief[k]===selectedWord)delete state.brief[k];});state.brief[b.dataset.slot]=selectedWord;selectedWord='';state.briefChecked=false;save();renderBrief();});
...
function renderPron(){
 const done=Object.keys(state.pron).length, speechOK=('speechSynthesis'in window)&&('SpeechSynthesisUtterance'in window);
 ...
 <div class="gate-grid">${pron.map((w,i)=>`<div class="gate-card"><div class="gate-head"><div class="gate-word"><span class="micro-label">CHECKPOINT ${i+1}</span><strong>${w.word}</strong><small>${w.ipa}</small></div><button type="button" class="listen-btn" data-say="${w.word}" ${speechOK?'':'disabled title="Audio is not available in this browser"'}>${speechOK?'🔊 Listen':'🔇 Audio unavailable'}</button></div><div class="choice-grid">${w.opts.map((o,j)=>`<button type="button" class="choice ${state.pron[w.id]&&j===w.correct?'correct':''}" data-pron="${w.id}" data-choice="${j}" ${state.pron[w.id]?'disabled':''}>${o}</button>`).join('')}</div><div class="feedback" id="fb-${w.id}">${state.pron[w.id]?w.note:''}</div></div>`).join('')}</div>${footer(`${done}/8 access checks cleared`,done===8,'Open client floor')}`);
 $$('[data-say]',mount).forEach(b=>b.onclick=()=>{if(!speak(b.dataset.say)){const card=b.closest('.gate-card'),fb=card&&$('.feedback',card);if(fb){if(!state.soundOn)fb.textContent='Sound is off. Turn sound on to hear the word.';else if(!speechOK)fb.textContent='Audio is not available in this browser. You can still use the IPA and stress patterns.';}}});
...
function renderCases(){
 ...
 <div class="folder-shelf">${ids.map(id=>`<button type="button" class="folder-btn ${state.casePanel===id?'active':''} ${caseComplete(id)?'done':''}" data-folder="${id}"><strong>${cases[id].icon} ${cases[id].label}</strong><small>${caseComplete(id)?'✓ File cleared':'Open file'}</small></button>`).join('')}</div>
...
function caseQuestion(id,step,q,opts,correct){const done=state.caseDone[id]?.['q'+step];return `<div style="margin-top:17px"><strong>${q}</strong><div class="choice-grid one">${opts.map((o,i)=>`<button type="button" class="choice ${done&&i===correct?'correct':''}" data-case="${id}" data-step="${step}" data-choice="${i}" ${done?'disabled':''}>${o}</button>`).join('')}</div><div class="feedback" id="casefb-${id}-${step}"></div></div>`;}
function renderLab(){
 ...
 <div class="lab-layout"><div class="data-wall">${labItems.map((x,i)=>`<button type="button" class="data-tile ${selected.includes(i)?'selected':''}" data-lab="${i}" ${state.labTrayOK?'disabled':''}><strong>${x[0]}</strong><small>${x[1]}</small></button>`).join('')}</div><div class="model-tray"><h3>MODEL TRAY</h3><p>${selected.length}/5 slots filled</p>${[0,1,2,3,4].map(i=>`<div class="tray-slot ${selected[i]!==undefined?'filled':''}">${selected[i]!==undefined?labItems[selected[i]][0]:'Empty input slot'}</div>`).join('')}</div></div>
 <div class="feedback" id="labFeedback">${state.labTrayOK?'Model tray accepted. A new alert just arrived.':''}</div>
 ${state.labTrayOK?`<div class="game-card" style="margin-top:18px"><span class="micro-label">ASSUMPTION SHOCK</span><h3>Repair costs suddenly rise by 18%.</h3><p>Which part of the model should be reviewed immediately?</p><div class="choice-grid one"><button type="button" class="choice ${state.labShock?'correct':''}" data-shock="0" ${state.labShock?'disabled':''}>Claim-cost / inflation assumptions</button><button type="button" class="choice" data-shock="1" ${state.labShock?'disabled':''}>The company logo</button><button type="button" class="choice" data-shock="2" ${state.labShock?'disabled':''}>The office seating plan</button></div><div class="feedback" id="shockFeedback">${state.labShock?'Correct. The cost assumptions must reflect the new environment.':''}</div></div>`:''}
 <div class="stage-footer"><div class="stage-score">${complete?'Risk Lab cleared':state.labTrayOK?'Tray accepted — solve the shock':'Select 5 model inputs'}</div><div class="stage-actions">${!state.labTrayOK?'<button type="button" class="secondary-game" id="checkLab">Run model check</button>':''}${completionButton(complete,'Send model note')}</div></div>`);
 $$('[data-lab]',mount).forEach(b=>b.onclick=()=>{const now=performance.now(),i=Number(b.dataset.lab);if(i===lastLabIndex&&now-lastLabAt<250)return;lastLabIndex=i;lastLabAt=now;if(state.labTrayOK)return;if(!selected.includes(i)&&selected.length>=5){sfx('bad');const fb=$('#labFeedback');if(fb)fb.textContent='The model tray is full. Remove one input before adding another.';return;}state.lab=selected.includes(i)?selected.filter(x=>x!==i):[...selected,i];save();renderLab();});
...
function renderInbox(){
 ...
 <div class="phone-frame"><div class="phone-screen"><div class="phone-head"><strong>Northstar · Team Inbox</strong><span>${done}/${inbox.length} checked</span></div><div class="mail-list">${inbox.map(m=>`<article class="mail"><div class="mail-top"><strong>${m.from}</strong><span>${m.tag}</span></div><p>${m.text}</p><div class="mail-actions"><button type="button" class="choice ${state.inbox[m.id]===true?'correct':''}" data-mail="${m.id}" data-v="true" ${state.inbox[m.id]!==undefined?'disabled':''}>✓ Keep</button><button type="button" class="choice ${state.inbox[m.id]===false?'correct':''}" data-mail="${m.id}" data-v="false" ${state.inbox[m.id]!==undefined?'disabled':''}>Archive</button></div><div class="feedback" id="mailfb-${m.id}">${state.inbox[m.id]!==undefined?m.why:''}</div></article>`).join('')}</div></div></div>
...
function renderMeeting(){
 ...
 mount.innerHTML=stageFrame(6,'Keep the client meeting on track','The client is here. This time the questions arrive one by one — answer clearly without overpromising or hiding behind jargon.','assets/session1/realistic/client-meeting.png',['Live conversation','Clear English','Decision support'],`${dispatch('🤝','Meeting live','A poor answer stalls the conversation; a clear one moves it forward. Focus on the clearest professional answer.')}
 <div class="meeting-scene"><div class="meeting-photo"><img src="assets/session1/realistic/client-meeting.png" alt="Actuarial consultant in a client meeting."></div><div class="chat-panel"><div class="chat-log">${transcript}${!complete?`<div class="bubble client"><strong>${turn.speaker}:</strong> ${turn.q}</div>`:''}</div>${!complete?`<div class="live-choice"><span class="micro-label">YOUR RESPONSE</span><div class="choice-grid one">${turn.answers.map((a,i)=>`<button type="button" class="choice" data-meet="${i}">${a[0]}</button>`).join('')}</div><div class="feedback" id="meetingFeedback">${state.meetingWrong||''}</div></div>`:`<div class="final-card"><span class="micro-label" style="color:#b8e4e5">MEETING RESULT</span><h3>Client confidence maintained</h3><p>You were useful without pretending that uncertainty can disappear.</p></div>`}</div></div>
 ${footer(`${idx}/${meeting.length} questions handled`,complete,'Close the meeting')}`);
 $$('[data-meet]',mount).forEach(b=>b.onclick=()=>{const now=performance.now();if(now<meetingLockUntil)return;meetingLockUntil=now+350;const i=Number(b.dataset.meet);$$('[data-meet]',mount).forEach(x=>x.disabled=true);if(turn.answers[i][1]){state.meeting.push(i);state.meetingWrong='';sfx('ok');save();renderMeeting();}else{state.meetingWrong=MEETING_WRONG;sfx('bad');save();renderMeeting();}});
 requestAnimationFrame(()=>{const log=$('.chat-log',mount);if(log)log.scrollTop=log.scrollHeight;});
...
function renderBoard(){
 ...
 mount.innerHTML=stageFrame(7,'Rescue the board slide','The meeting is over — almost. The final slide is unreadable actuarial jargon. Fix it before it appears on the boardroom screen.','assets/session1/realistic/boardroom.png',['Final mission','Jargon rescue','Board-ready English'],`${dispatch('🚨','Board slide emergency','Select a clear replacement from the deck, then click the line you want to replace. Four correct lines will rebuild the slide.')}
 <div class="slide-workbench"><div class="bad-slide"><span class="micro-label" style="color:#9ec5d3">ORIGINAL SLIDE</span><h3>What actuarial professionals deliver</h3><ul>${slideSlots.map(s=>`<li>${s.bad}</li>`).join('')}</ul></div><div class="good-slide"><span class="micro-label">YOUR BOARD-READY SLIDE</span><h3>What does an actuary do?</h3>${slideSlots.map((s,i)=>{const val=state.slide[s.id];let cls='slide-slot';if(val)cls+=' filled';if(state.slideChecked)cls+=val===s.answer?' correct':' wrong';return `<button type="button" class="${cls}" data-slide="${s.id}" ${state.earned[7]?'disabled':''}><span class="micro-label">LINE ${i+1}</span><div>${val?escapeHtml(val):'Choose a replacement line…'}</div></button>`;}).join('')}</div></div>
 <div class="replacement-deck">${replacements.map((r,i)=>`<button type="button" class="replacement ${selectedReplacement===r?'active':''} ${used.includes(r)?'used':''}" data-replace="${i}" aria-pressed="${selectedReplacement===r?'true':'false'}" ${state.earned[7]?'disabled':''}>${r}</button>`).join('')}</div>
 ...
 <div class="stage-footer"><div class="stage-score">${correct}/4 board lines correct</div><div class="stage-actions"><button type="button" class="secondary-game" id="checkSlide" ${state.earned[7]?'disabled':''}>Run clarity check</button>${completionButton(state.slideChecked&&correct===4,'Send to board')}</div></div>
 ...
 $$('[data-slide]',mount).forEach(b=>b.onclick=()=>{if(!selectedReplacement){const fb=$('#slideFeedback');if(fb)fb.textContent='Select a replacement line from the deck first.';return;}Object.keys(state.slide).forEach(k=>{if(state.slide[k]===selectedReplacement)delete state.slide[k];});state.slide[b.dataset.slide]=selectedReplacement;selectedReplacement='';state.slideChecked=false;save();renderBoard();});
...
function updateSoundButton(){
 ...
 const fxOK=!!(window.AudioContext||window.webkitAudioContext);
 if(!speechOK&&!fxOK){soundBtn.textContent='🔈 Audio unavailable';soundBtn.disabled=true;soundBtn.setAttribute('aria-disabled','true');return;}
 soundBtn.disabled=false;soundBtn.removeAttribute('aria-disabled');
 soundBtn.textContent=state.soundOn?'🔊 Sound on':'🔈 Sound off';soundBtn.setAttribute('aria-pressed',String(state.soundOn));
}
function render(){updateHUD();updateSoundButton();({1:renderBrief,2:renderPron,3:renderCases,4:renderLab,5:renderInbox,6:renderMeeting,7:renderBoard}[state.stage]||renderBrief)();requestAnimationFrame(keepActiveMapVisible);}
setModal(false);
render();
})();
