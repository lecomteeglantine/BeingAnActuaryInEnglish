...
const KEY='actuary_m1_s1_northstar_v16_audit';
...
function footer(score,nextEnabled,label='Mission complete'){const cleared=!!state.earned[state.stage];return `<div class="stage-footer"><div class="stage-score">${score}</div><div class="stage-actions"><button class="primary-game" id="completeMission" ${nextEnabled&&!cleared?'':'disabled'}>${cleared?'Mission already cleared':label+' →'}</button></div></div>`;}
...
function renderBrief(){
 ...
 <div class="stage-footer"><div class="stage-score">${correctCount}/8 lines correct</div><div class="stage-actions"><button class="secondary-game" id="checkBrief" ${state.earned[1]?'disabled':''}>Check brief</button><button class="primary-game" id="completeMission" ${correctCount===8&&!state.earned[1]?'':'disabled'}>${state.earned[1]?'Mission already cleared':'Send to manager →'}</button></div></div>`);
...
function renderBoard(){
 ...
 <div class="stage-footer"><div class="stage-score">${correct}/4 board lines correct</div><div class="stage-actions"><button class="secondary-game" id="checkSlide" ${state.earned[7]?'disabled':''}>Run clarity check</button><button class="primary-game" id="completeMission" ${correct===4&&!state.earned[7]?'':'disabled'}>${state.earned[7]?'Mission already cleared':'Send to board →'}</button></div></div>
