...
const roles={
  3:[
    ['Lead interviewer','Asks Past Simple follow-up questions.','L'],
    ['Experience interviewer','Asks Present Perfect questions.','E'],
    ['Candidate + evidence keeper','Answers from the cards; evidence is saved automatically.','C']
  ],
  4:[
    ['Lead interviewer','Opens the interview and leads the final decision.','L'],
    ['Experience interviewer','Asks Present Perfect questions.','E'],
    ['Follow-up interviewer','Asks Past Simple questions.','F'],
    ['Candidate','Answers from the candidate cards.','C']
  ]
};
...
function load(){
  try{
    ...
    const cleanGrammar={};
    if(state.grammarAnswers&&typeof state.grammarAnswers==='object'){
      Object.keys(state.grammarAnswers).forEach(k=>{
        const i=Number(k),v=state.grammarAnswers[k];
        if(Number.isInteger(i)&&i>=0&&i<grammar.length&&grammar[i].opts.includes(v))cleanGrammar[i]=v;
      });
    }
    state.grammarAnswers=cleanGrammar;
    const cleanQuestions=[]; const counts={pp:0,ps:0};
    if(Array.isArray(state.questions))state.questions.forEach(id=>{
      const q=qbank.find(x=>x.id===id&&x.good);
      if(q&&!cleanQuestions.includes(id)&&counts[q.type]<3&&cleanQuestions.length<6){cleanQuestions.push(id);counts[q.type]++;}
    });
    ...
    state.decision=['hire','nohire'].includes(state.decision)?state.decision:'';
    ['strength1','strength2','concern','pitchDraft'].forEach(k=>state[k]=typeof state[k]==='string'?state[k].slice(0,5000):'');
    state.timer=Number.isFinite(state.timer)?Math.max(0,Math.min(120,state.timer)):120;
    const grammarCorrect=grammar.every((g,i)=>state.grammarAnswers[i]===g.a);
    const pp=state.questions.filter(id=>qbank.find(q=>q.id===id)?.type==='pp').length;
    const ps=state.questions.filter(id=>qbank.find(q=>q.id===id)?.type==='ps').length;
    if(!state.players)state.step=0;
    else if(state.step>2&&!grammarCorrect)state.step=2;
    else if(state.step>3&&(pp!==3||ps!==3))state.step=3;
    else if(state.step>4&&state.roundsDone.length<6)state.step=4;
    else if(state.step>5&&(!state.decision||!state.strength1.trim()||!state.strength2.trim()||!state.concern.trim()))state.step=5;
...
function pitchNotes(){
  ...
  const evidenceText=notes.map(n=>n[1]).join(' ');
  const text=state.step===6&&state.pitchDraft.trim()?state.pitchDraft:evidenceText;
  const words=text.trim()?text.trim().split(/\s+/).length:0,sec=Math.round(words/100*60);
...
function clearAfterTeamChange(){
  state.questions=[]; state.roundsDone=[]; state.decision=''; state.strength1=''; state.strength2=''; state.concern=''; state.pitchDraft=''; state.timer=120; state.running=false;
}
function clearAfterGrammarChange(){
  state.questions=[]; state.roundsDone=[]; state.decision=''; state.strength1=''; state.strength2=''; state.concern=''; state.pitchDraft=''; state.timer=120; state.running=false;
}
function clearAfterQuestionChange(){
  state.roundsDone=[]; state.decision=''; state.strength1=''; state.strength2=''; state.concern=''; state.pitchDraft=''; state.timer=120; state.running=false;
}
...
function renderGrammar(){
  ...
  screen.innerHTML=`<p class="eyebrow">STEP 3 · QUICK CHECK</p><h2>Past Simple or Present Perfect?</h2><p class="screen-lead">Discuss, then choose. Get all 6 right to continue.</p><div class="quiz-list">${grammar.map((g,i)=>{const chosen=state.grammarAnswers[i];return `<div class="quiz-row"><p>${i+1}. ${esc(g.q)}</p><div class="mini-options">${g.opts.map(o=>`<button class="mini-option ${chosen===o?(o===g.a?'correct':'wrong'):''}" data-gi="${i}" data-val="${esc(o)}" type="button">${esc(o)}</button>`).join('')}</div><div class="feedback ${chosen?(chosen===g.a?'good':'bad'):''}">${chosen?(chosen===g.a?'✓ ':'✗ ')+esc(g.why):''}</div></div>`}).join('')}</div><p class="counter">Score: ${correct}/6</p>${actions('Choose interview questions →',correct<6)}`;
  document.querySelectorAll('[data-gi]').forEach(b=>b.addEventListener('click',()=>{const i=Number(b.dataset.gi),v=b.dataset.val;if(state.grammarAnswers[i]!==v){state.grammarAnswers[i]=v;clearAfterGrammarChange();}save();renderGrammar();update();}));bindNav(()=>{if(correct===6)next();});
}
function renderQuestions(){
  ...
  document.querySelectorAll('[data-qid]').forEach(b=>b.addEventListener('click',()=>{
    ...
    clearAfterQuestionChange(); save(); renderQuestions(); update();
  }));
  bindNav(()=>{if(pp===3&&ps===3)next();});
}
function interviewerFor(q){
...
function renderInterview(){
  ...
  screen.innerHTML=`<p class="eyebrow">STEP 5 · INTERVIEW</p><h2>Ask your 6 questions</h2><p class="screen-lead">Ask each question aloud. Reveal the answer, collect the evidence, then move on.</p>${qs.map((q,i)=>{const done=state.roundsDone.includes(i),locked=!done&&i!==nextIndex;return `<article class="interview-round ${i===nextIndex?'active':''} ${locked?'locked':''}"><div class="round-top"><div><span class="round-num">${i+1}</span><span class="round-type ${q.type}">${q.type==='pp'?'PRESENT PERFECT':'PAST SIMPLE'}</span></div><span class="speaker-badge">${esc(interviewerFor(q))}</span></div><p class="spoken-question">${esc(q.text)}</p><button class="interview-btn ghost reveal-btn" data-ri="${i}" type="button" ${locked?'disabled':''}>${done?'Answer revealed ✓':'Candidate: reveal answer'}</button><div class="answer-reveal" ${done?'':'hidden'}><strong>Candidate answer</strong><p>${esc(q.answer)}</p><small>Evidence: ${esc(q.note)}</small></div></article>`}).join('')}${actions('Make your decision →',state.roundsDone.length<6)}`;
...
function defaultPitch(){
  const dec=state.decision==='hire'?'hire Lerato':'not hire Lerato';
  const fit=state.decision==='hire'?'make her a strong candidate':'mean she is not the best fit yet';
  return `Our candidate is Lerato Mokoena, an actuarial analyst. She completed her undergraduate degree in 2022 and started working in insurance in 2023. In 2024, she failed one professional exam, reviewed her mistakes and retook it later.

Since 2023, she has worked on pricing and valuation projects. She has taken several professional actuarial exams and has learned to be more resilient under pressure.

Her main strengths are: ${state.strength1 || '[strength 1]'}; ${state.strength2 || '[strength 2]'}. One concern is: ${state.concern || '[concern]'}.

After discussing the evidence, we have decided to ${dec}. The main reason is that… Overall, her past experience and what she has learned so far ${fit}. Add one example from the interview before you present.`;
}
