(() => {
  'use strict';
  // M1 Session 2 museum engine · R8 functional audit

  const STORAGE_KEY = 'being-an-actuary-m1-session2-museum-r8';
  const LEGACY_KEYS = ['being-an-actuary-m1-session2-museum-r6','being-an-actuary-m1-session2-museum-r5','being-an-actuary-m1-session2-museum-r4','being-an-actuary-m1-session2-museum-r3','being-an-actuary-m1-session2-museum-r2','being-an-actuary-m1-session2-museum-r1'];
  const MAX_SCORE = 32;
  const galleryThumbs = [
  ...
  const sfxAssets = {
    on: 'assets/session2-museum/audio/sound-on.wav',
    select: 'assets/session2-museum/audio/select.wav',
    save: 'assets/session2-museum/audio/save.wav',
    complete: 'assets/session2-museum/audio/gallery-complete.wav',
    final: 'assets/session2-museum/audio/final.wav',
    timer: 'assets/session2-museum/audio/timer.wav',
    attention: 'assets/session2-museum/audio/attention.wav'
  };
  ...
  function saveGallery(gi) {
    ...
    playSfx(gi === galleries.length - 1 ? 'final' : 'complete');
    if (state.completed.every(Boolean)) {
      state.finalReached = true;
      state.reviewFromFinal = false;
      state.currentGallery = galleries.length - 1;
