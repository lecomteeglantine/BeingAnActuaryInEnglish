...
(function(){
  ...
  const KEY = 'actuarial_group_draft_r30';
  const OLD_KEYS = ['actuarial_group_draft_r29','actuarial_group_draft_r28','actuarial_group_draft_r27','actuarial_group_draft_r26','actuarial_group_draft_r25','actuarial_group_draft_r24','actuarial_group_draft_r23','actuarial_group_draft_r22','actuarial_group_draft_r21','actuarial_group_draft_v4_final_audit','actuarial_group_draft_v3_audit'];
  ...
  soundBtn.addEventListener('click', ()=>{
    ...
    if(!soundOn){
      try{ window.speechSynthesis && window.speechSynthesis.cancel(); }catch(_){}
      try{ const ctx = audioCtx; audioCtx = null; if(ctx && ctx.close) ctx.close().catch(()=>{}); }catch(_) { audioCtx = null; }
    }
    updateSound();
    save();
    if(soundOn) playFx('select');
  });
