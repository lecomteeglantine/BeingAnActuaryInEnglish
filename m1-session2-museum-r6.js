(() => {
  'use strict';
  // M1 Session 2 museum engine · R6 functional stability audit

  const STORAGE_KEY = 'being-an-actuary-m1-session2-museum-r6';
  const LEGACY_KEYS = ['being-an-actuary-m1-session2-museum-r5','being-an-actuary-m1-session2-museum-r4','being-an-actuary-m1-session2-museum-r3','being-an-actuary-m1-session2-museum-r2','being-an-actuary-m1-session2-museum-r1'];
  const MAX_SCORE = 32;
  const galleryThumbs = [
  ...
  function saveGallery(gi) {
    ...
    playSfx(gi === galleries.length - 1 ? 'final' : 'save');
    if (state.completed.every(Boolean)) {
      state.finalReached = true;
      state.reviewFromFinal = false;
      state.currentGallery = galleries.length - 1;
