...
(function(){
  ...
  const KEY = 'actuarial_group_draft_r21';
  const OLD_KEYS = ['actuarial_group_draft_v4_final_audit'];
  const mount = $('#gameMount');
  const progressFill = $('#progressFill');
  const dots = $$('#progressDots li');
  const soundBtn = $('#soundToggle');
  const defaultState = () => ({stage:1, soundOn:true, client:null, expertise:[], team:[], tools:[], identity:[], challengeChoice:null, firmName:'', promise:''});
  let state = normaliseState(load());
  ...
  const stageMeta = {
    1:{time:'09:00', title:'Choose your first target client', subtitle:'Your investors want to know who this new firm is for. Pick one client segment to anchor the whole draft.', img:'group-office-r21.png', icon:'🎯', badges:['Choose one','This decision shapes the alert','Discuss before selecting'], audio:'Round one. Choose one target client for your actuarial consultancy.', gallery:[['group-case-files-r21.png','Client files waiting for your decision'],['group-client-meeting-r21.png','Imagine the first client conversation'],['group-office-r21.png','Your team enters the consultancy']]},
    2:{time:'09:08', title:'Build your technical expertise', subtitle:'You cannot promise everything. Choose exactly two areas of expertise that make sense together.', img:'group-case-files-r21.png', icon:'📚', badges:['Choose exactly two','Technical offer','Stay strategic'], audio:'Round two. Choose two expertise areas that support your chosen client.', gallery:[['group-case-files-r21.png','Case files suggest several directions'],['group-analytics-r21.png','Think of the data you would analyse'],['group-risk-lab-r21.png','Models and scenarios must match the brief']]},
    3:{time:'09:16', title:'Recruit the core team', subtitle:'Your consultancy needs people, not just ideas. Choose the two profiles that will make the biggest difference.', img:'group-client-meeting-r21.png', icon:'👥', badges:['Choose exactly two','Human resources','Trade-offs matter'], audio:'Round three. Recruit two profiles for your new actuarial firm.', gallery:[['group-client-meeting-r21.png','Client-facing skills matter'],['group-office-r21.png','Who will work in this office?'],['group-boardroom-r21.png','Who can defend the work in the boardroom?']]},
    4:{time:'09:24', title:'Invest in the tools', subtitle:'The board will ask what your firm actually uses to analyse uncertainty. Pick two tools.', img:'group-risk-lab-r21.png', icon:'🛠️', badges:['Choose exactly two','Technology','Budget pressure'], audio:'Round four. Pick two tools that your firm will use to analyse uncertainty.', gallery:[['group-risk-lab-r21.png','The risk lab is open'],['group-analytics-r21.png','Dashboards and metrics matter'],['group-boardroom-r21.png','Choose tools you can justify']]},
    5:{time:'09:31', title:'Decide what kind of firm you are', subtitle:'These identity choices are free, but you will need to prove them later. Choose two.', img:'group-analytics-r21.png', icon:'✨', badges:['Choose exactly two','Free but important','Prepare the pitch'], audio:'Round five. Choose two identity words for your firm.', gallery:[['group-office-r21.png','The office creates a first impression'],['group-client-meeting-r21.png','Your identity must sound credible'],['group-boardroom-r21.png','The board will challenge vague promises']]},
    6:{time:'09:38', title:'War-room review', subtitle:'Check whether your firm is coherent, affordable and convincing before the real test arrives.', img:'group-boardroom-r21.png', icon:'🧭', badges:['Review','Budget check','Strengths + risks'], audio:'Round six. Review the firm before the investor board arrives.', gallery:[['group-boardroom-r21.png','Boardroom rehearsal'],['group-analytics-r21.png','Review your evidence'],['group-case-files-r21.png','Review your client fit']]},
    7:{time:'09:45', title:'Client alert', subtitle:'A real client problem hits your desk. The challenge is fixed by your choices — not by randomness.', img:'group-risk-lab-r21.png', icon:'🚨', badges:['Deterministic','Make one decision','Consequences'], audio:'Round seven. Respond to a client alert based on your earlier choices.', gallery:[['group-risk-lab-r21.png','You need evidence, not guesswork'],['group-client-meeting-r21.png','Prepare a clear response'],['group-boardroom-r21.png','Your answer must survive scrutiny']]},
    8:{time:'09:55', title:'Boardroom pitch', subtitle:'Name the firm, generate the board brief and prepare your final presentation.', img:'group-boardroom-r21.png', icon:'🏛️', badges:['Boardroom','Every student speaks','Copy your brief'], audio:'Round eight. Prepare the boardroom pitch and final brief.', gallery:[['group-boardroom-r21.png','Final presentation time'],['group-client-meeting-r21.png','Client language must stay clear'],['group-office-r21.png','Your consultancy is now ready to launch']]}
  };
  ...
  function normaliseState(raw){
    ...
    const valid = (group, ids) => [...new Set(Array.isArray(ids) ? ids : [])].filter(id => data[group].some(x=>x.id===id));
    const itemCost = (group,id) => optionBy(group,id)?.cost || 0;
    s.client = data.client.some(x=>x.id===s.client) ? s.client : null;
    s.expertise = valid('expertise', s.expertise).slice(0,2);
    s.team = valid('team', s.team).slice(0,2);
    s.tools = valid('tools', s.tools).slice(0,2);
    s.identity = valid('identity', s.identity).slice(0,2);
    s.challengeChoice = Number.isInteger(s.challengeChoice) && s.challengeChoice >= 0 && s.challengeChoice < 3 ? s.challengeChoice : null;
    s.firmName = String(s.firmName || '').slice(0,60);
    s.promise = String(s.promise || '').slice(0,220);
    const spentBeforeTools = (s.client ? itemCost('client',s.client) : 0) + s.expertise.reduce((n,id)=>n+itemCost('expertise',id),0) + s.team.reduce((n,id)=>n+itemCost('team',id),0);
    const total = spentBeforeTools + s.tools.reduce((n,id)=>n+itemCost('tools',id),0);
    let maxStage = 1;
    if(s.client) maxStage = 2;
    if(s.expertise.length===2) maxStage = 3;
    if(s.team.length===2 && 100-spentBeforeTools>=18) maxStage = 4;
    if(s.tools.length===2 && total<=100) maxStage = 5;
    if(s.identity.length===2 && total<=100) maxStage = 6;
    if(s.identity.length===2 && total<=100) maxStage = 7;
    if(s.challengeChoice===0 && s.identity.length===2 && total<=100) maxStage = 8;
  ...
  soundBtn.addEventListener('click', ()=>{ soundOn = !soundOn; updateSound(); save(); playFx('select'); });
  ...
  function selectionCount(){ return (state.client?1:0) + state.expertise.length + state.team.length + state.tools.length + state.identity.length; }
  function configCode(){
  ...
  function statusInfo(){
    ...
    return [left <= 10 ? 'High pressure' : 'Board-ready','status-good'];
  }
  function renderHud(){
    $('#budgetLeft').childNodes[0].nodeValue = String(budgetLeft());
    $('#budgetSpent').textContent = `${totalSpent()} points spent`;
    $('#budgetBar').style.width = `${Math.min(100, Math.max(0, totalSpent()))}%`;
    ...
    $('#configCode').textContent = `Config code · ${configCode()}`;
    $('#selectionsMade').textContent = `${selectionCount()} decisions made`;
    $('#currentStageCard').textContent = `Round ${state.stage} / 8`;
    const [label, cls] = statusInfo();
    const pill = $('#statusPill');
    pill.textContent = label; pill.className = `status-pill ${cls}`;
    dots.forEach((dot, i)=>{ dot.classList.toggle('active', i+1===state.stage); dot.classList.toggle('done', i+1<state.stage); });
    progressFill.style.width = `${((state.stage-1)/7)*100}%`;
  }
  ...
  function frame(stageNum, inner){
    ...
    return `<article class="stage-card"><div class="stage-head"><div><p class="round-time">${meta.time} · ROUND ${stageNum} OF 8</p><h2>${meta.title}</h2><p>${meta.subtitle}</p><div class="mini-badges">${meta.badges.map(b=>`<span>${b}</span>`).join('')}</div><div class="head-actions"><button type="button" class="secondary-btn round-speak" data-say="${escapeAttr(meta.audio)}">🔊 Hear this round</button><button type="button" class="secondary-btn" id="teamPromptBtn">💬 Team prompt</button></div></div><div class="stage-head-media"><img src="${meta.img}" alt="Illustration for stage ${stageNum}"></div></div><div class="stage-body"><div class="objective"><div class="icon">${meta.icon}</div><div><strong>Objective</strong><p>${meta.title}</p></div></div><div id="stageFeedback" class="feedback-box"></div><div class="stage-gallery">${meta.gallery.map(([src,cap])=>thumbHtml(src, cap)).join('')}</div>${inner}</div></article>`;
  }
  function cardHtml(group, item, selected, softLocked=false){
    ...
    return `<button type="button" class="choice-card ${selected?'selected':''} ${softLocked?'soft-locked':''}" data-group="${group}" data-id="${item.id}"><div class="top"><div class="emoji">${item.icon || '•'}</div><span class="cost-pill">${cost}</span></div><h3>${item.label}</h3><p>${item.blurb || ''}</p>${extra}${item.tag ? `<span class="card-tag">${item.tag}</span>`:''}</button>`;
  }
  function coachPrompt(){
    const prompts = {
      ...
      8:'Split the speaking roles so every student speaks for about two minutes.'
    };
  ...
  function renderStage4(){
    mount.innerHTML = frame(4, `<div class="notice-box"><strong>Task:</strong> Choose exactly <strong>2 tools</strong>. Expensive tools stay visible, but the game will block them if your remaining budget is too small.</div><div class="option-grid">${data.tools.map(item=>{ const selected=state.tools.includes(item.id); const softLocked=!selected && budgetLeft()<item.cost; return cardHtml('tools', item, selected, softLocked); }).join('')}</div><div class="decision-row"><div class="selection-meter">Choose <strong>2</strong> · Selected: ${state.tools.length}/2</div><div class="code-badge">Live budget · ${budgetLeft()} pts left</div></div><div class="nav-row"><button type="button" class="secondary-btn" id="backBtn">← Back</button><button type="button" class="primary-link" id="nextBtn" ${state.tools.length===2 && budgetLeft()>=0 ? '' : 'disabled'}>Lock tools →</button></div>`);
    bindRoundHelpers(); paintFeedback();
  ...
  function renderBoardroomBrief(){
    ...
    const box = $('#briefBox');
    if(!box) return;
    const ready = state.firmName.trim() && state.promise.trim();
    box.innerHTML = `<p class="module-number" style="margin:0">BOARDROOM BRIEF</p><h3 id="briefFirmName" style="margin-top:10px;color:#102a43">${escapeHtml(state.firmName || 'Your new firm')}</h3><p id="briefPromiseText">${escapeHtml(state.promise || 'Add a one-sentence promise to make the firm sound memorable and client-focused.')}</p><div class="brief-columns"><div><h4 style="color:#102a43">Firm DNA</h4><ul><li><strong>Target client:</strong> ${clientLabel}</li><li><strong>Expertise:</strong> ${state.expertise.map(id=>optionBy('expertise',id).label).join(' + ')}</li><li><strong>Team:</strong> ${state.team.map(id=>optionBy('team',id).label).join(' + ')}</li><li><strong>Tools:</strong> ${state.tools.map(id=>optionBy('tools',id).label).join(' + ')}</li><li><strong>Identity:</strong> ${state.identity.map(id=>optionBy('identity',id).label).join(' + ')}</li><li><strong>Budget:</strong> ${totalSpent()} / 100 points (${left} left)</li><li><strong>Config code:</strong> ${configCode()}</li></ul></div><div><h4 style="color:#102a43">How to defend the firm</h4><ul><li>Explain why this client needs actuarial support.</li><li>Show how your expertise and tools help you understand risk and uncertainty.</li><li>Justify your budget choices and trade-offs.</li><li>Summarise your response to the <strong>${challengeLabel}</strong>.</li><li>Use at least one strength and one weak point from the review.</li></ul></div></div><div class="student-grid"><div><strong>Student 1 · Who we are</strong><p>Firm name, promise and target client.</p></div><div><strong>Student 2 · What actuaries do</strong><p>Expertise, data, models, uncertainty and risk.</p></div><div><strong>Student 3 · How we built the firm</strong><p>Team, tools, costs and trade-offs.</p></div><div><strong>Student 4 · Why trust us?</strong><p>Identity, challenge response, strengths and weak points.</p></div></div>${ready ? `<div class="finish-banner"><strong>Boardroom access unlocked</strong><span>You now have a complete actuarial firm to present. In a 3-person group, Student 3 also explains the client alert response. Everyone still speaks for approximately 2 minutes.</span></div>` : `<div class="notice-box" style="margin-top:18px"><strong>One last step:</strong> add a firm name and a one-sentence promise to unlock the final boardroom brief.</div>`}`;
    const copyBtn = $('#copyBtn');
    if(copyBtn) copyBtn.disabled = !ready;
  }
  function renderStage8(){
    mount.innerHTML = frame(8, `<div class="notice-box"><strong>Final task:</strong> name your firm, give it a one-sentence promise, then use the generated brief to prepare your oral presentation. <strong>Every student speaks for approximately 2 minutes.</strong></div><div class="brief-grid"><label class="input-box">Firm name<input id="firmName" class="text-input" type="text" maxlength="60" placeholder="e.g. Northbridge Actuarial" value="${escapeHtml(state.firmName)}"></label><label class="input-box">One-sentence promise<textarea id="firmPromise" class="text-area" maxlength="220" placeholder="We help organisations understand uncertainty and make clear, evidence-based decisions.">${escapeHtml(state.promise)}</textarea></label></div><div class="firm-brief" id="briefBox"></div><div class="nav-row"><button type="button" class="secondary-btn" id="copyBtn" disabled>Copy our firm brief</button><button type="button" class="secondary-btn" id="restartBtn">Start a new firm</button></div>`);
    bindRoundHelpers(); paintFeedback();
  ...
  function copyBrief(){
    ...
    const copied = () => { setFeedback('Your firm brief has been copied.', 'success'); paintFeedback(); playFx('copy'); };
    const fallback = () => {
      ...
      try { const ok = document.execCommand('copy'); if(ok) copied(); else { setFeedback('Copy failed. Select the boardroom brief and copy it manually.', 'warning'); paintFeedback(); playFx('error'); } } catch(_) { setFeedback('Copy failed. Select the boardroom brief and copy it manually.', 'warning'); paintFeedback(); playFx('error'); }
      ta.remove();
    };
  ...
  function toggleChoice(group, id, limit){
    ...
    if(!clicked) return;
    if(group==='client'){
      if(state.client !== id) state.challengeChoice = null;
      state.client = id;
      setFeedback(`Client selected: ${clicked.label}.`, 'success');
      playFx('select');
      save(); renderStage1(); return;
    }
    const arr = state[group];
    const idx = arr.indexOf(id);
    if(idx >= 0){ arr.splice(idx,1); setFeedback(`${clicked.label} removed.`, 'info'); playFx('select'); }
    else if(arr.length >= limit){ setFeedback(`You already chose ${limit}. Remove one option first.`, 'warning'); playFx('error'); }
    else if(group==='tools' && budgetLeft() < (clicked.cost || 0)){ setFeedback('Not enough budget for this tool. Choose a cheaper option or revise earlier choices.', 'warning'); playFx('error'); }
    else { arr.push(id); setFeedback(`${clicked.label} added to the draft.`, 'success'); playFx('select'); }
    if(group==='expertise' || group==='client') state.challengeChoice = null;
    save(); render();
  }
  function canEnter(stage){
    ...
    if(stage===7) return budgetLeft() >= 0;
    if(stage===8) return state.challengeChoice !== null && challenge().choices[state.challengeChoice][1];
    return true;
  }
  function go(stage){
    if(lockNav) return;
    ...
    lockNav = true;
    clearFeedback();
    state.stage = stage; save(); render(); playFx('next');
    const target=document.querySelector('.progress-panel'); if(target) target.scrollIntoView({behavior:'smooth',block:'start'});
    setTimeout(()=>{ lockNav = false; }, 180);
  }
  function render(){
    renderHud();
    switch(state.stage){
