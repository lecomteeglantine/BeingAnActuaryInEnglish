...
(function(){
  ...
  const KEY = 'actuarial_group_draft_r28';
  const OLD_KEYS = ['actuarial_group_draft_r27','actuarial_group_draft_r26','actuarial_group_draft_r25','actuarial_group_draft_r24','actuarial_group_draft_r23','actuarial_group_draft_r22','actuarial_group_draft_r21','actuarial_group_draft_v4_final_audit','actuarial_group_draft_v3_audit'];
  ...
  function renderHud(){
    ...
    dots.forEach((dot, i)=>{
      const round = i + 1;
      const active = round === state.stage;
      dot.classList.toggle('active', active);
      dot.classList.toggle('done', round < state.stage);
      if(active) dot.setAttribute('aria-current','step'); else dot.removeAttribute('aria-current');
      dot.setAttribute('aria-label', `Round ${round}${active ? ', current' : round < state.stage ? ', completed' : ''}`);
    });
