(()=>{
'use strict';
const AUDIO='assets/m2-radio-r44/audio/';
const ASSET='assets/m2-radio-r44/';
const STORAGE='actuarial-radio-r44-state';
...
const mission2=[
 ...
 {audio:'m2-underwriting.mp3',word:'underwriting',answer:0,options:['<strong>UN</strong>-der-writ-ing','un-<strong>DER</strong>-writ-ing','un-der-<strong>WRIT</strong>-ing','un-der-writ-<strong>ING</strong>'],feedback:'In the British-English noun “underwriting”, the main stress falls on the first syllable; “writ” carries secondary stress.'},
...
const posters=[
 {id:'flood',title:'Flood risk',small:'Catastrophe modelling · storm exposure',img:'assets/m2-radio-r44/poster-flood-r44.svg',pills:['Property','Storm']},
 {id:'life',title:'Life cover',small:'Protecting a family’s financial future',img:'assets/m2-radio-r44/poster-life-r44.svg',pills:['Life','Protection']},
 {id:'motor',title:'Motor claims',small:'Claims handling after an accident',img:'assets/m2-radio-r44/poster-motor-r44.svg',pills:['Motor','Claims']},
 {id:'pension',title:'Pension',small:'Retirement income · longevity',img:'assets/m2-radio-r44/poster-pension-r44.svg',pills:['Retirement','Longevity']}
];
const finalQs=[
...
let activeAudio=null; let finalTimer=null; let finalRunId=0; const activeSfx=new Set(); let startLocked=false;

function loadState(){
 try{
  const raw=localStorage.getItem(STORAGE); if(!raw)return;
  const s=JSON.parse(raw); if(!s || !Number.isInteger(s.mission))return;
  const safeMission=Math.min(5,Math.max(0,s.mission));
  const safeLives=Number.isInteger(s.lives)?Math.min(3,Math.max(1,s.lives)):3;
  const safeCorrect=Number.isFinite(s.correct)?Math.max(0,Math.floor(s.correct)):0;
  const safeTotal=Number.isFinite(s.total)?Math.max(safeCorrect,Math.floor(s.total)):0;
  const safeRestore=Number.isFinite(s.restore)?Math.min(100,Math.max(0,Math.floor(s.restore))):0;
  const safeBadges=Array.isArray(s.badges)?s.badges.filter(b=>Object.prototype.hasOwnProperty.call(BADGE_META,b)):[];
  state={...state,...s,mission:safeMission,lives:safeLives,correct:safeCorrect,total:safeTotal,restore:safeRestore,badges:[...new Set(safeBadges)],effects:s.effects!==false,finished:s.finished===true};
  if(state.finished){state.mission=5;state.restore=100;els.start.textContent='View your completed broadcast →'}
  else if(state.mission>0){els.start.textContent='Continue the control room →';}
 }catch(_e){try{localStorage.removeItem(STORAGE)}catch(_ignore){}}
}
...
function stopSfx(){activeSfx.forEach(a=>{try{a.pause();a.currentTime=0;}catch(_e){}});activeSfx.clear();}
function playSfx(file){if(!state.effects)return; const a=new Audio(AUDIO+file);activeSfx.add(a);a.volume=.6;const cleanup=()=>activeSfx.delete(a);a.addEventListener('ended',cleanup,{once:true});a.addEventListener('error',cleanup,{once:true});a.play().catch(()=>cleanup());}
function showAudioError(button){const host=button?.closest('.challenge-card,.clip-chip,.final-gate')||els.mount; if(host&&!host.querySelector('.audio-error')){const p=document.createElement('p');p.className='audio-error';p.textContent='Audio could not play. Check browser volume and make sure the assets/m2-radio-r44/audio folder was uploaded.';host.appendChild(p);}announce('Audio could not play.');}
...
function visualStrip(cards){return `<div class="mission-extra-strip">${cards.map(c=>`<div class="mini-visual-card"><span class="visual-emoji" aria-hidden="true">${c.emoji}</span><strong>${c.title}</strong><small>${c.small}</small></div>`).join('')}</div>`}
function callerRoster(){const callers=[['👩🏽','London pricing'],['🧔🏼','Dublin pensions'],['👨🏾','Toronto reserving'],['👩🏻','Sydney catastrophe']];return `<div class="caller-roster">${callers.map(([face,label])=>`<div class="caller-avatar"><span class="avatar-face" aria-hidden="true">${face}</span><strong>${label.split(' ')[0]}</strong><small>${label.replace(label.split(' ')[0]+' ','')}</small></div>`).join('')}</div>`}
...
function missionComplete(index,score,total,badgeId){
 ...
 els.mount.innerHTML=stageFrame(`SYSTEM ${index+1} RESTORED`,'Mission clear',index===5?'The station is live.':`You repaired this part of the studio with ${score}/${total} first-try answers.`,'assets/m2-radio-r44/'+(['mission-lost-r44.svg','mission-stress-r44.svg','mission-call-r44.svg','mission-interference-r44.svg','mission-adverts-r44.svg','mission-final-r44.svg'][index]),'Illustrated repaired studio system',`<div class="mission-clear"><div class="clear-icon">${index===5?'🔴':'✅'}</div><h3>${index===5?'You saved the broadcast!':'System restored'}</h3><p>${index===5?'Actuarial Radio International is back on air.':`Broadcast restoration is now at <strong>${state.restore}%</strong>.`}</p><div class="mission-stats"><span>First try: ${score}/${total}</span><span>Overall accuracy: ${state.total?Math.round(state.correct/state.total*100):100}%</span></div>${index<5?nextButton(nextLabel):'<button type="button" class="radio-primary" data-replay>Replay the whole game</button>'}</div>`);
 const n=els.mount.querySelector('[data-next]');if(n)n.addEventListener('click',e=>{const btn=e.currentTarget;if(btn.disabled)return;btn.disabled=true;state.mission=index+1;updateHUD();saveState();renderMission(index+1);window.scrollTo({top:els.shell.offsetTop-20,behavior:'smooth'});});
...
function renderMission1(){genericMission({index:0,items:mission1,kicker:'MISSION 1 · RECOVER THE ARCHIVE',title:'Lost Audio Files',desc:'Five programme files have lost their labels. Hear the recording and restore the correct actuarial filename.',img:'assets/m2-radio-r44/mission-lost-r44.svg',alt:'Illustration of lost audio files and a reel in a radio archive',scene:{icon:'💾',text:'The archive server is online, but the labels are gone.',prompt:'Which filename belongs to this audio?',audioLabel:'Play lost file',gallery:visualStrip([{emoji:'📁',title:'File names',small:'life · risk · pricing'},{emoji:'🎛️',title:'Studio deck',small:'listen before you click'},{emoji:'📉',title:'Risk terms',small:'insurance & modelling'},{emoji:'🧩',title:'Restore labels',small:'match sound to meaning'}])},renderOptions:item=>item.options.map(o=>`<button type="button" class="answer-btn" data-answer="${o}">${o}</button>`).join(''),badgeTest:s=>s>=4?'ears':null});}
function renderMission2(){genericMission({index:1,items:mission2,kicker:'MISSION 2 · REPAIR THE MIX',title:'Fix the Word Stress',desc:'The stress markers were wiped from the pronunciation board. Listen for the strongest syllable and put the marker back.',img:'assets/m2-radio-r44/mission-stress-r44.svg',alt:'Illustration of a mixing desk and sound levels for word stress',scene:{icon:'🎚️',text:'Do not say the word. Just listen for the beat that carries the main stress.',prompt:'Where is the main stress?',audioLabel:'Hear the word',optionClass:'stress-options',gallery:visualStrip([{emoji:'🥁',title:'Beat 1',small:'hear the pulse'},{emoji:'📈',title:'Stress peak',small:'one syllable is stronger'},{emoji:'🎙️',title:'No speaking',small:'listening only'},{emoji:'⚡',title:'Quick choice',small:'lock the best pattern'}])},renderOptions:item=>item.options.map((o,i)=>`<button type="button" class="answer-btn stress-btn" data-answer="${i}">${o}</button>`).join(''),badgeTest:s=>s>=4?'stress':null});}
function renderMission3(){
 playSfx('sfx-phone.mp3');
 genericMission({index:2,items:mission3,kicker:'MISSION 3 · ANSWER THE DESK',title:"Who's Calling?",desc:'Client calls are coming in. Identify the key actuarial term hidden inside each natural sentence.',img:'assets/m2-radio-r44/mission-call-r44.svg',alt:'Illustration of an incoming telephone call in the radio studio',scene:{icon:'☎️',text:'Incoming line. The caller will not repeat the message automatically — use Replay if you need it.',prompt:'Which key actuarial term did you hear?',audioLabel:'Answer the call',gallery:callerRoster()},renderOptions:item=>item.options.map(([icon,label])=>`<button type="button" class="caller-choice" data-answer="${label}"><span aria-hidden="true">${icon}</span>${label}</button>`).join('')});
}
function renderMission4(){playSfx('sfx-static.mp3');genericMission({index:3,items:mission4,kicker:'MISSION 4 · CLEAN THE SIGNAL',title:'Signal Interference',desc:'Four international feeds are buried under static. Decode the key word, then unlock the clean transmission.',img:'assets/m2-radio-r44/mission-interference-r44.svg',alt:'Illustration of a noisy radio waveform and signal interference',scene:{icon:'📡',text:'Static is deliberate. Listen for the stressed vowel and the consonant shape.',prompt:'Which key term survived the interference?',sub:'Correct answer = clean studio feed unlocked.',audioLabel:'Play noisy feed',gallery:visualStrip([{emoji:'📶',title:'Noisy feed',small:'hear through static'},{emoji:'🔍',title:'Target word',small:'catch the key term'},{emoji:'🧼',title:'Clean version',small:'unlocks after success'},{emoji:'🎯',title:'Focus',small:'consonants + stressed vowel'}])},renderOptions:item=>item.options.map(o=>`<button type="button" class="answer-btn" data-answer="${o}">${o}</button>`).join(''),extraCorrect:(item,root)=>{const extra=root.querySelector('[data-extra]');extra.insertAdjacentHTML('afterbegin',`<div class="static-banner"><strong>Clean feed unlocked.</strong> Compare it with the noisy version.</div>${audioConsole(item.clean,'Play clean feed')}`);bindAudio(extra);},badgeTest:s=>s>=3?'signal':null});}
function renderMission5(){
 let selected=null,matches=0,firstScore=0;const missed=new Set();
 function render(){
  els.mount.innerHTML=stageFrame('MISSION 5 · RECONNECT THE AD BREAK','The Wrong Adverts','The audio clips and visual campaigns have been unplugged. Listen to a clip, select it, then connect it to the correct poster.','assets/m2-radio-r44/mission-adverts-r44.svg','Illustration of four different insurance advertising posters',`<div class="scene-strip"><span class="scene-icon" aria-hidden="true">🔌</span><p>Four loose audio cables. Four visual campaigns. Patch every cable into the right channel.</p></div><p class="cable-note"><strong>How to play:</strong> play a clip → select its cable → click the matching poster.</p>${visualStrip([{emoji:'🎙️',title:'Ad break',small:'4 clips to reconnect'},{emoji:'🖼️',title:'Poster wall',small:'look for the right campaign'},{emoji:'🔌',title:'Patch cables',small:'select then connect'},{emoji:'🎧',title:'Replay freely',small:'listen as often as needed'}])}<div class="patch-board"><div><p class="question-counter">AUDIO CABLES</p><div class="clip-list">${adverts.map(a=>`<div class="clip-chip" data-clip="${a.id}"><button type="button" class="play-audio" data-audio="${a.audio}" aria-label="Play ${a.label}">▶ ${a.label}</button><span class="clip-label"><strong>${a.label}</strong><span>Listen, then patch the right poster.</span></span><button type="button" class="clip-select" data-select="${a.id}">Patch</button></div>`).join('')}</div></div><div><p class="question-counter">VISUAL CHANNELS</p><div class="poster-list">${posters.map(p=>`<button type="button" class="poster-card poster-rich" data-poster="${p.id}"><span class="poster-thumb"><img src="${p.img}" alt="${p.title} poster illustration"></span><span class="poster-copy"><strong>${p.title}</strong><small>${p.small}</small><span class="poster-meta">${p.pills.map(x=>`<span class="poster-pill">${x}</span>`).join('')}</span></span></button>`).join('')}</div></div></div><div class="feedback-box" data-feedback>Select an audio cable, then a poster.</div>`);
  bindAudio();
...
function runFinal(){
 ...
 const renderQ=()=>{
  ...
  buttons.forEach(btn=>btn.addEventListener('click',()=>{
   if(btn.disabled||runId!==finalRunId)return;
   buttons.forEach(b=>b.disabled=true);stopAudio();
   const ok=String(item.answer)===btn.dataset.finalAnswer;recordAttempt(ok);answered++;
   if(ok){score++;btn.classList.add('correct');fb.className='feedback-box good';fb.textContent='✅ Clean hit.';playSfx('sfx-success.mp3');}
   else{btn.classList.add('wrong');const backup=loseFuse();fb.className='feedback-box bad';fb.textContent=`Signal miss.${backup?' Backup power restored two fuses.':''}`;}
   const isLast=q===finalQs.length-1;
   if(isLast)stopFinalTimer();
   setTimeout(()=>{if(runId!==finalRunId)return;if(isLast){finishFinal(score,runId);return;}q++;renderQ();},520);
  }));
 };
 finalTimer=setInterval(()=>{if(runId!==finalRunId){stopFinalTimer();return;}time--;updateTimerVisual(time);if(time>0&&time<=10)playSfx('sfx-tick.mp3');if(time<=0){stopFinalTimer();finalTimeout(score,answered,runId);}},1000);
 renderQ();
}
...
function finalTimeout(score,done,runId){if(runId!==undefined&&runId!==finalRunId)return;finalRunId++;stopFinalTimer();stopAudio();els.mount.innerHTML=stageFrame('FINAL MISSION · SIGNAL DROPPED','Broadcast delayed','The clock reached zero before all eight transmissions were checked. Nothing is lost: the station holds the repaired systems while you retry the live test.','assets/m2-radio-r44/mission-final-r44.svg','Illustration of the final on-air countdown',`<div class="mission-clear"><div class="clear-icon">⏱️</div><h3>${score}/${done} correct before time</h3><p>${done} of ${finalQs.length} transmissions were completed. Use the retry to restart only the final 60-second challenge.</p><button type="button" class="radio-primary hot" id="retryFinal">Retry the live test →</button></div>`);document.getElementById('retryFinal').addEventListener('click',runFinal);}
function finishFinal(score,runId){if(runId!==undefined&&runId!==finalRunId)return;finalRunId++;stopFinalTimer();state.restore=100;state.finished=true;state.mission=5;if(score>=6)addBadge('hero');playSfx('sfx-onair.mp3');updateHUD();saveState();els.mount.innerHTML=stageFrame('BROADCAST RESTORED','You’re live!','Actuarial Radio International is broadcasting again.','assets/m2-radio-r44/mission-final-r44.svg','Illustration of the final on-air broadcast',`<div class="onair-card"><div class="onair-sign">ON AIR</div><h3>Broadcast saved.</h3><p>You repaired all six systems using listening, pronunciation discrimination and actuarial vocabulary.</p><div class="final-score">Final sprint: ${score}/${finalQs.length} · Overall accuracy: ${state.total?Math.round(state.correct/state.total*100):100}%</div><div class="mission-stats"><span>🎧 Audio archive restored</span><span>🎚️ Stress desk repaired</span><span>📡 Signal clean</span><span>🔌 Ad break patched</span></div><button type="button" class="radio-primary" data-replay>Replay the whole game</button></div>`);els.mount.querySelector('[data-replay]').addEventListener('click',resetGame);announce('Actuarial Radio is live. Game complete.');}
function renderFinished(){state.restore=100;updateHUD();els.mount.innerHTML=stageFrame('BROADCAST RESTORED','You’re live!','This browser has already completed the Actuarial Radio mission.','assets/m2-radio-r44/mission-final-r44.svg','Illustration of an on-air radio studio',`<div class="onair-card"><div class="onair-sign">ON AIR</div><h3>Broadcast saved.</h3><p>Your saved progress shows a completed mission.</p><div class="final-score">Overall accuracy: ${state.total?Math.round(state.correct/state.total*100):100}%</div><button type="button" class="radio-primary" data-replay>Replay the whole game</button></div>`);els.mount.querySelector('[data-replay]').addEventListener('click',resetGame);}
...
function resetGame(){finalRunId++;stopAudio();stopSfx();stopFinalTimer();startLocked=false;els.start.disabled=false;state={mission:0,lives:3,correct:0,total:0,restore:0,badges:[],effects:state.effects,finished:false};try{localStorage.removeItem(STORAGE)}catch(_e){}els.start.textContent='Enter the control room →';els.brief.hidden=false;els.shell.hidden=true;updateHUD();window.scrollTo({top:0,behavior:'smooth'});}

els.start.addEventListener('click',()=>{if(startLocked)return;startLocked=true;els.start.disabled=true;els.brief.hidden=true;els.shell.hidden=false;playSfx('sfx-jingle.mp3');renderMission(state.mission);setTimeout(()=>window.scrollTo({top:els.shell.offsetTop-12,behavior:'smooth'}),50);setTimeout(()=>{els.start.disabled=false;startLocked=false;},250)});
els.sound.addEventListener('click',()=>{state.effects=!state.effects;if(!state.effects)stopSfx();els.sound.setAttribute('aria-pressed',String(state.effects));els.sound.textContent=state.effects?'🔊 Sound effects on':'🔇 Sound effects off';saveState();if(state.effects)playSfx('sfx-success.mp3');});
els.restart.addEventListener('click',()=>{if(els.shell.hidden||window.confirm('Restart Actuarial Radio from Mission 1?'))resetGame();});
loadState();els.sound.setAttribute('aria-pressed',String(state.effects));els.sound.textContent=state.effects?'🔊 Sound effects on':'🔇 Sound effects off';updateHUD();
})();
