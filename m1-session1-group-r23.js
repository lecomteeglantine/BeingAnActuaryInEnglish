...
(function(){
  ...
  const KEY = 'actuarial_group_draft_r23';
  const OLD_KEYS = ['actuarial_group_draft_r22','actuarial_group_draft_r21','actuarial_group_draft_v4_final_audit','actuarial_group_draft_v3_audit'];
  ...
  const stageMeta = {
    ...
    2:{title:'Build your technical expertise', subtitle:'You cannot promise everything. Choose exactly two areas of expertise that make sense together.', img:'group-case-files-r21.png', icon:'📚', badges:['Choose exactly two','Technical offer','Stay strategic'], audio:'Round two. Choose two expertise areas that support your chosen client.', gallery:[['group-case-files-r21.png','Case files suggest several directions'],['group-analytics-r21.png','Think of the data you would analyse'],['group-risk-lab-r21.png','Models and scenarios must match the brief']]},
    ...
    5:{title:'Decide what kind of firm you are', subtitle:'These identity choices are free, but you will need to prove them later. Choose two.', img:'group-analytics-r21.png', icon:'✨', badges:['Choose exactly two','Free but important','Prepare the pitch'], audio:'Round five. Choose two identity words for your firm.', gallery:[['group-office-r21.png','The office creates a first impression'],['group-client-meeting-r21.png','Your identity must sound credible'],['group-boardroom-r21.png','The board will challenge vague promises']]},
    ...
    8:{title:'Boardroom pitch', subtitle:'Name the firm, generate the board brief and prepare your final presentation.', img:'group-boardroom-r21.png', icon:'🏛️', badges:['Boardroom','Every student speaks','Copy your brief'], audio:'Round eight. Prepare the boardroom pitch and final brief.', gallery:[['group-boardroom-r21.png','Final presentation time'],['group-client-meeting-r21.png','Client language must stay clear'],['group-office-r21.png','Your consultancy is now ready to launch']]}
  };
  ...
  function normaliseState(raw){
    ...
    if(s.challengeChoice!==null && ![0,1,2].includes(s.challengeChoice)) s.challengeChoice=null;
  ...
  function renderStage7(){
    ...
    if(state.challengeChoice!==null && c.choices[state.challengeChoice] && c.choices[state.challengeChoice][1]){
  ...
  function go(stage){
    if(lockNav) return;
    if(!canEnter(stage)){
      setFeedback('Complete the required choices on this round before continuing.', 'warning');
      paintFeedback(); playFx('error');
      return;
    }
    lockNav = true;
    clearFeedback();
  ...
  window.addEventListener('resize',()=>requestAnimationFrame(keepActiveProgressVisible),{passive:true});
  ...
  window.addEventListener('pagehide', ()=>{ try{ window.speechSynthesis && window.speechSynthesis.cancel(); }catch(_){} });
