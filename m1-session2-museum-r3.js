(() => {
  'use strict';
  // M1 Session 2 museum engine · R3 mega stability + clarity audit

  const STORAGE_KEY = 'being-an-actuary-m1-session2-museum-r3';
  const LEGACY_KEYS = ['being-an-actuary-m1-session2-museum-r2','being-an-actuary-m1-session2-museum-r1'];
  const MAX_SCORE = 32;
  const galleryThumbs = [
  ...
  function updateSaveBadge() {
    ...
    if (!badge) return;
    badge.textContent = storageAvailable ? '💾 Local auto-save' : '⚠️ Auto-save unavailable';
    badge.title = storageAvailable ? 'Progress is saved only in this browser.' : 'This browser is blocking local storage. Keep this tab open until you finish.';
  }
  ...
  function syncGalleryCompletion(gi) {
    ...
    state.completed[gi] = Boolean(saved && allAnswered(gi) && gallerySignature(gi) === saved);
  ...
  function renderGallery() {
    ...
    galleryView.innerHTML = `
      ...
      <section class="pitch-builder-card" aria-labelledby="pitchBuilderHeading">
        ...
        <div class="pitch-tools"><small id="galleryPitchCount">${countWords(state.pitchNotes[gi])} words · aim for 30–45</small><button class="museum-btn subtle" id="suggestPitchBtn" type="button" ${allAnswered(gi) ? '' : 'disabled'}>Use suggested wording</button></div>
    ...
    $('galleryPitchCount').className = initialPitchWords > 45 ? 'pitch-count-warning' : initialPitchWords >= 30 ? 'pitch-count-good' : '';
    note.addEventListener('input', () => {
      ...
      $('galleryPitchCount').textContent = `${words} words · aim for 30–45`;
      $('galleryPitchCount').className = words > 45 ? 'pitch-count-warning' : words >= 30 ? 'pitch-count-good' : '';
      renderProgress();
      renderNotebook();
    });
